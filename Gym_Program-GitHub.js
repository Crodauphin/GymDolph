...
//
// 19 August 2026 — cardioPool logFields (engine v0.9.15, no schema change, contract stays v1.3):
//   - ⚠️  SUPERSEDED same day: an earlier version of this pass added `difficulty` to `rowing`
//     based on an ambiguous prompt reading. That was wrong — Albin's actual intent is that
//     NEITHER the rower NOR the recumbent bike has a difficulty setting. Both corrected to
//     `logFields: ['duration']`. Exactly four pool items now carry `difficulty`: stairs,
//     skierg, upbike, ellip. commute and tread unchanged.
//   - D5 Mobility block: added missing `icon: '🧘'` (was falling back to the generic 💪 default
//     since its `type` is `'main'` for engine reasons)
//
// 24 August 2026 — cardioPool logFields (engine v0.9.18 session, no schema change, contract v1.3):
//   - `skierg`: logFields ['duration', 'difficulty'] -> ['duration']. Same correction pattern as
//     rowing/recbike — SkiErg has no difficulty setting to log. id/name/pinned unchanged.
//     Historic SkiErg entries keep their logged difficulty value; nothing retro-stripped.
//   - Stairmaster, upbike, ellip unchanged — three pool items now carry `difficulty`, not four.

// 29 August 2026 (no schema/engine change — content-only, no version bump needed):
//   - D6 activities: added 'strength' chip (🏋️, minutes-only — id+label+icon, no free/rest/
//     presets extras), placed after 'climb'
//   - cardioPool reordered: skierg and recbike moved permanently to the end (Albin rarely uses
//     either). New order: commute, stairs, rowing, tread, upbike, ellip, skierg, recbike
//   - `upbike`: logFields ['duration','difficulty'] -> ['duration'] — no bike captures a
//     difficulty level. `recbike` and `commute` unchanged (recbike was already duration-only).
//     `stairs` and `ellip` keep `difficulty` — not bikes, this correction doesn't reach them.
//     Historic upbike entries with a logged level are unaffected — only new logs change.
...
window.PROGRAM = {
  ...
  cardioPool: [
    ...
    { id: 'upbike',  name: 'Upright / racing bike', pinned: false, logFields: ['duration'] },
    { id: 'ellip',   name: 'Elliptical',        pinned: false, logFields: ['duration', 'difficulty'] },
    { id: 'skierg',  name: 'SkiErg',            pinned: false, logFields: ['duration'] },
    { id: 'recbike', name: 'Recumbent bike',    pinned: false, logFields: ['duration'] }
  ],
  ...
  days: [
    ...
    // ── D5 · Fri · Yoga / Mobility ──────────────────────────────────────────
    {
      ...
      blocks: [
        { id: 'main', type: 'main', bodyPart: 'Mobility', icon: '🧘', duration: '60 min',
          logType: 'session',
          exercises: [
