...
let state = {
  ...
  // v0.9.18: {weekKey, choice:'yes'|'no', accepted:[weekKey,…]}.
  // weekKey/choice are THIS week's answer and are overwritten every week.
  // `accepted` is the durable part: every week actually deloaded, in order. It is
  // a list and not a single lastAcceptedWeek string because turning a deload week
  // OFF has to roll the floor back, and a single string has nothing to roll back
  // to. Last accepted is then derived from the list, never typed (#12).
  deloadDecision: null, // v0.9.12 (#5) · accepted[] added v0.9.18
...
(function () {
  ...
  // v0.9.19: `key(i)` and `length` are deliberately NOT wrapped — they still walk the
  // whole origin and return fully-prefixed keys. That asymmetry is what broke the
  // storage readout: a caller that enumerates with key(i) gets 'gd_x__gymdolph_history'
  // and then feeds it straight back to the wrapped getItem(), which prefixes it a
  // SECOND time and returns null. Exposed so anyone enumerating can strip the
  // namespace instead of inferring its shape (#11), and read through raw.get.
  window.__gdNS     = NS;
  window.__gdRawGet = raw.get;
})();
...
function load() {
  ...
  try {
    ...
    // v0.9.18 migration: installs before this release stored only {weekKey, choice}
    // and kept no record of past deloads. If the stored answer is a 'yes', that one
    // week is recoverable; anything earlier is not, because nothing ever wrote it.
    // The app therefore behaves on first run as though no deload has been taken —
    // the trigger can fire immediately if the preceding weeks qualify. Seeding a
    // fake floor to suppress that would be inventing history (#12).
    if (state.deloadDecision && !Array.isArray(state.deloadDecision.accepted)) {
      state.deloadDecision.accepted =
        state.deloadDecision.choice === 'yes' && state.deloadDecision.weekKey
          ? [state.deloadDecision.weekKey] : [];
    }
    ...
    if (savedWk === thisWk) {
      ...
      // v0.9.21d — THE BOARD KEEPS ITS OWN KEY IN BOTH DIRECTIONS. "Keep the board
      // the user moved to" only fired for AHEAD. For BEHIND — Albin declined the
      // Monday rollover to add two yoga days to the previous week — the key was left
      // at the real week while the board still held last week's sessions. Every
      // autosave then stamped `recordDateFor(dayId, thisWk)`: last week's edits got
      // this week's dates, the existing-record lookup (keyed by weekKey) missed and
      // wrote a DUPLICATE D3, and "Save & move to next week" added one to an already
      // advanced key and landed on W38 — a Monday dated seven days into the future.
      // The sessions on the board belong to savedWk. The key says so now. The
      // pending prompt still fires; it is the flag, not the key, that drives it.
      state.currentWeekKey = savedWk;
    } else {
...
// v0.9.19 ONE-TIME, AND NOT REVERSIBLE (Albin's call, 27 Aug): delete the empty
// history records already on disk. Going forward none are written; this clears the
// ones the previous writers left behind — auto-ticked rest days, end-of-week swept
// passive days, and any day finished with nothing logged.
// The test is two conditions, not one:
//   · `inProgress` records are KEPT whatever they contain. A day opened and
//     abandoned is shown UNFINISHED; that is the whole point of keeping it.
//   · everything else needs recordHasContent(), which as of this release finally
//     includes cardio — without that fix this pass would have deleted real
//     sessions where only the treadmill or the commute was logged.
// The streak is recomputed afterwards because it is being redefined in the same
// release: past figures change regardless of this purge, and a stored count from
// the old definition must not survive it.
const MIGR_KEY_EMPTY_RECS = 'gymdolph_migr_emptyrecs_v0919';

// v0.9.19. Repairs and cleans a history array. IDEMPOTENT and therefore safe to run
// on every restore, not just once at upgrade — which it must be, because
// `state.history = data.history || []` in the Drive restore and the JSON import
// both replace history wholesale with a file that may predate this release. The
// migration key lives in localStorage and would already be set, so a one-shot pass
// meant a single restore silently reversed the whole thing and there was no second
// chance to clean it (#38b: remove the failure mode, don't guard it).
//
// Returns { removed, repaired } so callers can report; mutates state.history.
function repairAndPurgeHistory() {
  // STEP 1 — repair `inProgress` before reading it (Albin confirmed, 27 Aug).
  // Until v0.9.18a, reopening a finished day via Review and navigating away rebuilt
  // the record and flipped `inProgress` back to true on a session already done.
  // Twelve records in the live history carry 16-26 sets and ~70 minutes while still
  // flagged in progress; the most recent is 19 August and there is not one
  // content-bearing in-progress record after the fix shipped. For a record WITH
  // content that flag is damage, not meaning, and every reader added in this release
  // would otherwise inherit it. One of those records is why the streak broke at 19-08.
  //
  // THE `!r.dayType` GUARD IS WHAT MAKES THIS IDEMPOTENT, and it is the reason the
  // stamp was worth adding beyond the day-type question it was introduced for.
  // Only pre-v0.9.19 records lack the stamp, so a session you genuinely abandon
  // half-way from here on — sets logged, never finished — is stamped, and re-running
  // this can never quietly mark it complete. Scoped by a field that travels with the
  // record rather than by a typed cutoff date (#12), so it survives backup/restore.
  let repaired = 0;
  (state.history || []).forEach(r => {
    if (r && r.inProgress && !r.dayType && recordHasContent(r)) { delete r.inProgress; repaired++; }
  });
  // STEP 2 — drop empty records. Always safe to re-run: it only ever removes records
  // that the current rules would refuse to write in the first place.
  const before = (state.history || []).length;
  state.history = (state.history || []).filter(r => {
    if (!r) return false;
    if (recordHasContent(r)) return true;
    // Empty. Survives only as an abandoned TRAINING day — the one case where
    // "you started this and didn't finish" is worth showing. On a passive day it
    // goes, whether it was auto-ticked or tapped into by hand.
    return !!r.inProgress && recordDayType(r) === 'training';
  });
  return { removed: before - state.history.length, repaired };
}

// ── v0.9.20 (B4) — COLLAPSE THE DUPLICATE SET STORE ───────────────────
// Records written before this release carry every set twice (see recSetsForEntry).
// This pass makes `record.setLogs` the only copy. IDEMPOTENT, and run after every
// restore for the same reason the v0.9.19 pass is (#53): a Drive restore or JSON
// import replaces history wholesale with a file that may predate the change.
//
// PROMOTE BEFORE STRIPPING. The audit of the 29-08 export found zero entries
// holding sets the canonical map lacked, so on Albin's history this branch never
// fires — but Emily's history has never been seen by anyone, and a blind strip
// there would be a guess (#38: prefer the lookup that can miss).
//
// Then SURFACE ORPHANS. Five arrays in the live history sit in `record.setLogs`
// under an id with no `exercises[]` entry — 14 real sets, counted in tonnage and
// rendered nowhere (#49). They are there because the PROGRAM changed underneath
// the record: the id-to-name mapping that existed at write time is precisely what
// was destroyed. NO NAME IS INVENTED. The same day-scoped id has meant different
// exercises at different times (D2/m5 was High Row, then Assisted pull-up; D3/m4
// was Leg Extension, then Leg curl), and two of the five appear nowhere at all —
// so every route to a name either misses or answers confidently and wrongly
// (#11, #31, #38). The entry declares `orphan: true` and is labelled by its id.
// Albin resolves them by hand from History → Modify → ✕.
function normaliseHistorySetLogs() {
  let promoted = 0, stripped = 0, surfaced = 0, touched = 0;
  const asArr = v => (v == null) ? [] : (Array.isArray(v) ? v : Object.values(v));
  (state.history || []).forEach(r => {
    if (!r) return;
    let dirty = false;
    if (!r.setLogs || typeof r.setLogs !== 'object' || Array.isArray(r.setLogs)) {
      if (r.setLogs !== undefined) dirty = true;
      r.setLogs = {};
    }
    (r.exercises || []).forEach(ex => {
      if (!ex || !Object.prototype.hasOwnProperty.call(ex, 'setLogs')) return;
      const legacy = asArr(ex.setLogs).filter(Boolean);
      const canon  = asArr(r.setLogs[ex.id]).filter(Boolean);
      if (legacy.length > canon.length) {
        r.setLogs[ex.id] = JSON.parse(JSON.stringify(asArr(ex.setLogs)));
        promoted++;
      }
      delete ex.setLogs; stripped++; dirty = true;
    });
    // Orphans. Skipped once an entry exists, which is what keeps this idempotent.
    const known = new Set((r.exercises || []).map(e => e && e.id));
    Object.keys(r.setLogs).forEach(id => {
      if (known.has(id)) return;
      const arr = asArr(r.setLogs[id]).filter(Boolean);
      if (!arr.length) return;
      r.exercises = r.exercises || [];
      r.exercises.push({
        id, name: 'Unmatched — ' + id, checked: true, orphan: true,
        sets: arr.length,
        // Summary derived from the SET SHAPE, not from a program entry that no
        // longer exists — entryMode() reads the stored fields (#11).
        weight: arr.map(st => st.seconds != null ? holdLabel(st)
                          : (st.weight > 0 ? st.weight + 'kg×' + st.reps
  ...
  return { promoted, stripped, surfaced, touched };
}

// One-time pass at upgrade. The streak is recomputed because it is redefined in the
// same release: past figures change regardless of the purge, and a stored count from
// the old definition must not survive it.
function purgeEmptyHistoryRecords() {
  try { if (localStorage.getItem(MIGR_KEY_EMPTY_RECS)) return; } catch(e) { return; }
  try {
    const before = (state.history || []).length;
    const { removed, repaired } = repairAndPurgeHistory();
    try { localStorage.setItem(MIGR_KEY_EMPTY_RECS, String(Date.now())); } catch(e) {}
    if (removed || repaired) { recalcStreak(); save(); }
    console.info(`[v0.9.19] purged ${removed} empty record(s) of ${before}; ` +
                 `repaired inProgress on ${repaired} finished session(s)`);
  } catch (e) { console.warn('purgeEmptyHistoryRecords failed', e); }
}

// v0.9.20 (§6). THE RECEIPT. The v0.9.19 migration deleted 17 records and rewrote
// 12 flags and reported it to a console an installed PWA does not have; the only
// reason the result could be checked at all was that the storage box happens to
// show a session count. A one-time irreversible pass has to say what it did where
// the user can read it — same rule as the storage readout (#12, #13). Every number
// below is MEASURED off the array before and after, never typed.
//
// Queued rather than shown: at this point in boot the splash is still covering the
// screen, so an alert here would fade out unread behind it.
const MIGR_KEY_SETLOG_SHAPE = 'gymdolph_migr_setlogs_v0920';
function migrateSetLogShape() {
  try { if (localStorage.getItem(MIGR_KEY_SETLOG_SHAPE)) return; } catch(e) { return; }
  try {
    const bytesBefore = JSON.stringify(state.history || []).length;
    const { promoted, surfaced, touched } = normaliseHistorySetLogs();
    try { localStorage.setItem(MIGR_KEY_SETLOG_SHAPE, String(Date.now())); } catch(e) {}
    if (!touched) return;
    save();
    const bytesAfter = JSON.stringify(state.history || []).length;
    const saved = Math.max(0, bytesBefore - bytesAfter);
    const lines = [
      (state.history || []).length + ' sessions checked.',
      '• Duplicate set copies removed from ' + touched + ' session' + (touched===1?'':'s'),
      '• Storage reclaimed: ' + Math.round(saved / 1024) + ' KB',
    ];
    if (promoted) lines.push('• Sets recovered into the main record: ' + promoted);
    if (surfaced) lines.push('• Unmatched exercises now visible in History: ' + surfaced +
                             '\n  (sets logged against a program entry that no longer\n' +
                             '   exists — open History → Modify to keep or delete)');
    state._pendingMigrationReceipt = lines.join('\n');
  } catch (e) { console.warn('migrateSetLogShape failed', e); }
}

// v0.9.21 ONE-TIME. recordHasContent() now reads the declared `rest: true` flag, so
// repairAndPurgeHistory() would refuse to write these records today — but it is
// gated behind the v0.9.19 key, which is already set on every upgraded device, so
// it will never run again by itself. This clears the ones already on disk: 18-07
// and 25-07 in Albin's history, both `activeLog: {nothing:{mins:''}}`, both showing
// as '0 exercises · 0 sets · —'.
//
// NO SEPARATE HOOK IS NEEDED for Drive restore or JSON import: repairRestoredHistory()
// already calls repairAndPurgeHistory() on both paths, and that function now applies
// the new rule. #53 is satisfied by the rule living in the shared helper rather than
// in the one-time pass — which is the shape that rule was asking for.
//
// Receipt APPENDS rather than overwrites. A device coming from v0.9.19 straight to
// v0.9.21 — Emily's, most likely — runs the v0.9.20 pass and this one in the same
// boot, and the second must not silently eat the first one's numbers.
const MIGR_KEY_RESTONLY = 'gymdolph_migr_restonly_v0921';
function migrateRestOnlyActiveDays() {
  try { if (localStorage.getItem(MIGR_KEY_RESTONLY)) return; } catch(e) { return; }
  try {
    const { removed, repaired } = repairAndPurgeHistory();
    try { localStorage.setItem(MIGR_KEY_RESTONLY, String(Date.now())); } catch(e) {}
    if (!removed && !repaired) return;
    recalcStreak();
    save();
    const line = '• Rest-only days removed from History: ' + removed;
    state._pendingMigrationReceipt = state._pendingMigrationReceipt
      ? state._pendingMigrationReceipt + '\n' + line
      : (state.history || []).length + ' sessions checked.\n' + line;
    console.info(`[v0.9.21] purged ${removed} empty/rest-only record(s), repaired ${repaired}`);
  } catch (e) { console.warn('migrateRestOnlyActiveDays failed', e); }
}

// v0.9.21d ── RECORD CONSISTENCY REPAIR ───────────────────────────────────────
// Shared helper: idempotent, evidence-based, runs on the one-time key below AND
// inside repairRestoredHistory() so a Drive restore or JSON import gets it too (#53).
// Every rule reads a declared field and fixes a field DERIVED from it. Nothing is
// inferred from names or loads.
//
//  R1  weekKey must equal getWeekKey(date). A record's date is dateForWeekDay(weekKey,
//      dayId) at write time, so date determines weekKey. Albin re-dated three records
//      by hand and only `date` moved (Modify wrote one field); their weekKey said W37
//      for a W36 date, which is what threw the streak to 0.
//  R2  A record with content keyed AHEAD of the real current week is impossible —
//      nobody trains next week. Re-key to the real week, re-date for its dayId.
//      (The declined-rollover bug produced a Monday dated seven days out.)
//      NOT "dated after its own creation": 17 legitimate records are, because a
//      day opened early autosaves with its scheduled date. Measured before writing.
//  R3  dateStr regenerated from date, every record. History no longer renders it,
//      but older consumers still read it and it must not contradict `date`.
//  R4  deload retro-stamp from deloadDecision.accepted[] — declared list, never loads.
//  R5  Duplicates (same date + dayId) are COUNTED and reported, never deleted.
//      Which one is real is Albin's call, and Modify can delete either.
function repairRecordConsistency() {
  const out = { rekeyed: 0, aheadFixed: 0, dateStrFixed: 0, deloadStamped: 0, duplicates: 0 };
  const realWk = getWeekKey(new Date());
  const seen = {};
  (state.history || []).forEach(r => {
    if (!r || !r.date) return;
    // R2 first: it rewrites both date and weekKey, so R1 then finds them consistent.
    if (r.weekKey && r.weekKey > realWk && recordHasContent(r)) {
      const nd = dateForWeekDay(realWk, r.dayId);
      if (nd) { r.weekKey = realWk; r.date = nd; out.aheadFixed++; }
    }
    // R1
    const wk = getWeekKey(new Date(r.date + 'T12:00:00'));
    if (wk && r.weekKey !== wk) { r.weekKey = wk; out.rekeyed++; }
    // R3
    const ds = recordDateStr(r.date);
    if (r.dateStr !== ds) { r.dateStr = ds; out.dateStrFixed++; }
    // R4
    if (!r.deload && isDeloadWeek(r.weekKey)) { r.deload = true; out.deloadStamped++; }
    // R5
    const k = r.date + '|' + r.dayId;
    if (seen[k]) out.duplicates++; else seen[k] = true;
  });
  return out;
}
const MIGR_KEY_RECORD_CONSISTENCY = 'gymdolph_migr_reckey_v0921d';
function migrateRecordConsistency() {
  try { if (localStorage.getItem(MIGR_KEY_RECORD_CONSISTENCY)) return; } catch(e) { return; }
  try {
    const realWk = getWeekKey(new Date());
    const keyWasAhead = !!(state.currentWeekKey && state.currentWeekKey > realWk);
    const r = repairRecordConsistency();
    // The board key itself: only pulled back when R2 actually fired on a record
    // keyed to it — that is the evidence the "ahead" was the bug and not a
    // deliberate Saturday move-ahead, which leaves no content-bearing record.
    let keyFixed = false;
    if (keyWasAhead && r.aheadFixed > 0) { state.currentWeekKey = realWk; keyFixed = true; }
    try { localStorage.setItem(MIGR_KEY_RECORD_CONSISTENCY, String(Date.now())); } catch(e) {}
    const touched = r.rekeyed + r.aheadFixed + r.dateStrFixed + r.deloadStamped + (keyFixed ? 1 : 0);
    if (!touched && !r.duplicates) return;
    recalcStreak();
    save();
    const lines = [];
    if (r.aheadFixed)    lines.push('• Sessions dated into next week, moved back: ' + r.aheadFixed);
    if (r.rekeyed)       lines.push('• Week keys corrected from their dates: ' + r.rekeyed);
    if (r.dateStrFixed)  lines.push('• Date labels resynced: ' + r.dateStrFixed);
    if (r.deloadStamped) lines.push('• Records marked as deload week: ' + r.deloadStamped);
    if (keyFixed)        lines.push('• Home board returned to this week');
    if (r.duplicates)    lines.push('• Same day logged twice (check History → Modify): ' + r.duplicates);
    const body = lines.join('\n');
    state._pendingMigrationReceipt = state._pendingMigrationReceipt
      ? state._pendingMigrationReceipt + '\n' + body
      : (state.history || []).length + ' sessions checked.\n' + body;
    console.info('[v0.9.21d] record consistency', r, keyFixed ? '(board key reset)' : '');
  } catch (e) { console.warn('migrateRecordConsistency failed', e); }
}

// Called after ANY wholesale replacement of history — Drive restore, JSON import.
// Reports to the console rather than interrupting: the restore already has its own
// success dialog, and a second modal about record hygiene would be noise.
function repairRestoredHistory(source) {
  try {
    // v0.9.20: the set-log collapse runs here too, and silently. A restore has its
    // own success dialog and the shape change is not the user's business twice;
    // the RECEIPT is for the one-time upgrade pass, which is the irreversible one.
    const shape = normaliseHistorySetLogs();
    if (shape.touched) console.info(`[v0.9.20] ${source}: normalised ${shape.touched} record(s), ` +
                                    `promoted ${shape.promoted}, surfaced ${shape.surfaced}`);
    const { removed, repaired } = repairAndPurgeHistory();
    // v0.9.21d: week keys, dates, deload stamps — same rules as the one-time pass.
    const rc = repairRecordConsistency();
    const rcTouched = rc.rekeyed + rc.aheadFixed + rc.dateStrFixed + rc.deloadStamped;
    if (removed || repaired || shape.touched || rcTouched) {
      recalcStreak();
      console.info(`[v0.9.19] ${source}: purged ${removed} empty record(s), ` +
                   `repaired inProgress on ${repaired} session(s)`);
      if (rcTouched) console.info(`[v0.9.21d] ${source}: record consistency`, rc);
    }
  } catch (e) { console.warn('repairRestoredHistory failed', e); }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  purgeEmptyHistoryRecords();   // v0.9.19 one-time, after load() has populated history
  migrateSetLogShape();         // v0.9.20 one-time, AFTER the purge — no point
                                // collapsing set logs on records about to be deleted
  migrateRestOnlyActiveDays();  // v0.9.21 one-time — it re-runs the purge under
                                // the new rest-day rule, and appends to the receipt
  migrateRecordConsistency();   // v0.9.21d one-time, LAST — week keys, dates, deload
                                // stamps, and the board key itself; appends too
  ...
  window.dismissSplash = function() {
    ...
    setTimeout(() => {
      ...
      // v0.9.20 (§6): the migration receipt goes first and hands the week reset on
      // through its onClose, so neither dialog can overwrite the other.
      if (state._pendingMigrationReceipt) {
        const _m = state._pendingMigrationReceipt; state._pendingMigrationReceipt = null;
        setTimeout(() => appAlert('HISTORY UPDATED', _m, () => {
          if (state._pendingWeekReset) setTimeout(() => showWeekResetModal(), 300);
        }), 400);
      } else if (state._pendingWeekReset) { setTimeout(() => showWeekResetModal(), 400); }
...
function migrateHistoryDates() {
  try {
    ...
    // v0.9.21d — RETIRED. This pass trusts weekKey and rewrites date from it. That
    // was right for pre-v0.9.10 records, where `date` was the clock at Finish, and
    // it has run on every device that ever held such a record. Since v0.9.10 the
    // writer derives date from weekKey, so the two only ever disagree when a date
    // was EDITED in Modify — and there `date` is the intent. Running this over a
    // fresh install or a restore (its key is not exported) undid Albin's three
    // hand-corrected dates before repairRecordConsistency() could see them. The
    // consistency repair holds the opposite, correct, invariant (weekKey from date),
    // so this one now marks itself done and does nothing (#38b).
    try { localStorage.setItem('gymdolph_dateMigration', 'v1'); } catch(e) {}
    return;
    let changed = 0;
    state.history.forEach(r => {
    ...
    if (changed) {
      save();   // v0.9.20 (B6): was a bare single-key write
...
function cleanupPhantomRecords() {
  try {
    ...
    // are both future-dated and empty, which can never be legitimate.
    // v0.9.19, TWO CHANGES:
    //  1. The emptiness test was `!setCount(r)`, which is blind to cardio for the
    //     reasons set out on recordHasContent(). A future-dated day carrying a
    //     logged commute and nothing else was deletable. Same test everywhere now.
    //  2. The old comment justified sparing PAST empty records by pointing at
    //     autoCloseFinishedWeeks() writing real ones. That writer is gone, and past
    //     empty records are handled once by purgeEmptyHistoryRecords() instead.
    // SCOPE (#15): future-dated records are still removed even when in-progress.
    // "Kept and shown UNFINISHED" is about days that HAPPENED; a day that has not
    // arrived yet is a phantom regardless of which flag it carries.
    const today = todayStr();
    const before = state.history.length;
    state.history = state.history.filter(r => !(r && r.date > today && !recordHasContent(r)));
...
function setCount(rec) {
  if (!rec) return 0;
  // v0.9.20 (B4): counts the CANONICAL map, per exercise entry, via the one
  // resolver. The old version summed exercises[].setLogs and fell back to the
  // top-level map only when that produced zero — a fallback that could never
  // fire while both copies existed, and would have counted map KEYS (exercises)
  // rather than sets if it had (#19: listing a consumer is not checking it).
  return (rec.exercises || [])
    .reduce((n, ex) => n + recSetsForEntry(rec, ex).filter(Boolean).length, 0);
}
...
function autoCloseFinishedWeeks() {
  try {
    ...
    // v0.9.19: THE HISTORY WRITE IS GONE. This was the fifth record-construction
    // site, writing `autoTicked` + `autoLabel` rows for last week's passive days.
    // Those rows are exactly what this release deletes, so re-creating them every
    // Monday would undo the migration on the next app open.
    // The `exists` guard that used to sit above went with it, and that matters: it
    // skipped a day whose record already existed, so once empty records stopped
    // being written it would have matched LESS often and the sweep would have
    // started writing replacements. Deleting the writer removes the failure mode
    // instead of guarding it (#38b) — there is no longer anything to guard.
    // What remains is the weekSessions mirror, which is the only part the user can
    // see. `autoLabel` no longer has a writer anywhere in the engine.
    if (sweptDayIds.length) {
      ...
      console.info(`[v0.9.19] auto-ticked ${sweptDayIds.length} passive day(s) of ${closedWk} on the board (no history written)`);
    }
...
// v0.9.21d — THE DATE THIS SESSION WILL BE FILED UNDER, on the workout screen.
// Derived from the board's week key and the day id — the exact inputs
// recordDateFor() uses — so it is the record's date shown before the record is
// written (#13, validate on show). Had this line existed a week ago it would have
// read 'Monday 14 September' on the 7th and the week-key fault would have been
// caught on sight instead of in History. When editing a past record it shows
// that record's date instead, which is the one being edited.
function workoutDateLine(day) {
  if (!day) return '';
  let iso = null;
  if (state._editingHistoryId) {
    const r = state.history.find(h => h.id === state._editingHistoryId);
    if (r && r.date) iso = r.date;
  }
  if (!iso) iso = dateForWeekDay(state.currentWeekKey, day.id);
  if (!iso) return '';
  const d = new Date(iso + 'T12:00:00');
  if (isNaN(d)) return '';
  const DOW = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
  const MON = ['January','February','March','April','May','June','July','August','September','October','November','December'];
  return `${DOW[d.getDay()]} ${d.getDate()} ${MON[d.getMonth()]}`;   // 'Monday 7 September', locale-independent
}
function setWorkoutHeader(day) {
  document.getElementById('wd-day-label').textContent = day.id;
  document.getElementById('wd-focus').textContent = day.name;
  const dl = document.getElementById('wd-date');
  if (dl) dl.textContent = workoutDateLine(day);
}
// ── REBUILD WORKOUT VIEW (restores all state) ──
function rebuildWorkoutView() {
  const day = state.workoutDay;
  if (!day) return;
  setWorkoutHeader(day);
...
// v0.9.21 — ACTIVE DAY MINUTES ARE A DURATION. An Active Day has no blocks, so
// neither stop trigger can ever fire on it (both filter day.blocks to main +
// secondary and find none) — the clock is never offered a stop, and if it never
// ran the record stored 0 against a 60-minute walk. 22-08 is the case: activeLog
// held {walk:{mins:60, preset:'Short', km:5}} and the record read '—'.
//
// Read from the DECLARED minutes, never inferred: the preset writes `mins` from
// the program's own `presets: [{label:'Short', mins:60, km:5}]` and a hand-typed
// entry writes the same field (#11). `rest: true` activities are skipped — they
// are the absence of a session, not a zero-length one.
function activeSecsFor(day, sess) {
  if (!day || !sess) return 0;
  // v0.9.21d — YOGA TOO. The 15/30/60 pick lived in sess.yogaSessionLog.duration,
  // became the display string '60 min · Combination' on the exercise entry, and
  // never reached the record's duration: 28-08 read 0m against a 60-minute
  // session. Same class as the walk below, same fill, same priority (below the
  // clock and below a stored measurement), same ~ marking. The sweep Albin asked
  // for: cardio-pool and commute minutes already flow through cardioSecsFor();
  // Active Day activities through the branch below; this was the one path left.
  if (day.type === 'stretch') {
    const yl = sess.yogaSessionLog;
    const m = yl ? +yl.duration : 0;
    return (!isNaN(m) && m > 0) ? Math.round(m * 60) : 0;
  }
  if (day.type !== 'cardio_day') return 0;
  ...
  (activeDayActivities(day) || []).forEach(a => {
    ...
    const m = +rec.mins;
    if (!isNaN(m) && m > 0) mins += m;
  });
  return Math.round(mins * 60);
}
function sessionDurationParts(day, sess) {
  let strength = getTimerSecs();
  // v0.9.21 — A DECLARED VALUE BEATS A DERIVED ONE. Once the STRENGTH SESSION
  // OVER? prompt has been answered YES, `strengthSecs` is a fact about the session
  // and the clock is no longer the witness. Read before the ceiling clamp, so a
  // stamped value is still subject to the same sanity bound as a measured one.
  // Cleared by resumeTimer(): deliberately restarting the clock after declaring
  // the strength half over contradicts the declaration, and the later act wins.
  if (sess && sess.strengthSecs != null) {
    const st = +sess.strengthSecs;
    if (!isNaN(st) && st >= 0) strength = st;
  }
  ...
  // `active` is carried, NOT added here. It is a fallback, and it has to be
  // applied BELOW the keep-guard in keptDurationParts() — see the note there.
  const active = activeSecsFor(day, sess);
  return { strength: strength, cardio: cardio, active: active,
           total: strength + cardio, clamped: clamped };
}
...
// v0.9.19 — THE STREAK READS FINISHED TRAINING DAYS, FULL STOP (Albin, 25 Aug).
//
// It used to build a Set of EVERY record date and walk back until one was missing.
// That cut both ways. Empty rest records were load-bearing — they were the only
// thing keeping the chain unbroken across Sunday — so deleting them would have
// snapped every streak. And an IN-PROGRESS record counted just as hard as a
// finished one, which is how opening D3 to test something and touching a single
// control added a day (observed live, 25 Aug).
//
// Walking PROGRAM's day types instead REMOVES the dependency rather than guarding
// it (#38b), and fixes the inflation in the same change. Three rules:
//   · a day whose PROGRAM type is not 'training' is STEPPED OVER — it neither
//     counts nor breaks. Rest is prescribed, not missed.
//   · a training day counts only if it has a record that is neither inProgress
//     nor autoTicked — declared fields, not inferred shape (#42).
//   · a training day with no such record BREAKS the chain.
function recalcStreak() {
  const finished = new Set();
  let oldest = null;
  (state.history || []).forEach(h => {
    if (!h || !h.date) return;
    if (h.inProgress || h.autoTicked) return;
    finished.add(h.date);
    if (!oldest || h.date < oldest) oldest = h.date;
  });
  let streak = 0;
  const d = new Date();
  // A day still open today must not break the chain — start at yesterday, as before.
  if (!finished.has(localDateStr(d))) d.setDate(d.getDate() - 1);
  // Floored at the oldest finished record so the walk terminates: with no training
  // day anywhere in PROGRAM the loop would otherwise step backwards forever.
  while (oldest && localDateStr(d) >= oldest) {
    const day = PROGRAM[DOW_TO_PROGRAM[d.getDay()]];
    if (day && day.type === 'training') {
      if (!finished.has(localDateStr(d))) break;
      streak++;
    }
...
// v0.9.20 (C7, rescoped). MEASURED, not estimated: one render of D4 — the worst
// day at 12 exercises — called this 5,669 times against 53 history records, and
// those calls resolved to just 123 DISTINCT STRINGS. Seven regex passes each, on
// input that repeats. 7.42 ms per render today, ~28 ms projected at 200 records.
//
// The roadmap proposed memoising getMostUsedSetValue() instead. That would have
// needed invalidation on every save(), every history edit and every delete — a
// new failure mode to maintain, when the underlying cost can simply be removed
// (#38b). Caching HERE needs no invalidation at all, because the function is pure:
// the same string has always produced the same answer and always will.
//
// The cache is bounded by the number of distinct exercise names that have ever
// been passed in — 123 today, and it only ever grows when the program gains a
// name. Measured after: 1.59 ms per render, 4.7x faster; ~6 ms at 200 records.
const _normExNameCache = new Map();
function normExName(name) {
  const raw = String(name || '');
  const hit = _normExNameCache.get(raw);
...
// ── v0.9.20 (B4): ONE PLACE THAT RESOLVES A RECORD'S SETS ─────────────
// Until now every record stored its set data TWICE: `record.setLogs`, a map
// keyed by exercise id, and a second copy on each `record.exercises[i].setLogs`.
// 23.1% of history by size, and worse than redundant — confirmHistorySet() only
// ever wrote to the top-level map, so a set corrected in History left the
// per-exercise copy stale and every reader below kept serving the old number to
// the coach and to the "last time" prefill. Measured on the 29-08 export: 260
// populated duplicate pairs, 0 disagreeing (the drift never fired), 0 entries
// holding sets the map lacked. The duplicate is deleted rather than kept in
// sync (#38b: remove the failure mode, don't guard it).
//
// `record.setLogs` is canonical. The per-exercise copy is read ONLY when the
// canonical map has no key for that id at all — an un-migrated record arriving
// from a restored backup (#53). ABSENCE, not emptiness: an empty canonical array
// means "the sets here were deleted", and falling back on that would resurrect
// them.
function recSetsForEntry(rec, ex) {
  if (!rec || !ex) return [];
  const top = rec.setLogs || {};
  const sl = Object.prototype.hasOwnProperty.call(top, ex.id) ? top[ex.id] : ex.setLogs;
  if (!sl) return [];
  return Array.isArray(sl) ? sl : Object.values(sl);
}
// Matched by normalised name, as every caller below did individually.
function recSetsFor(rec, exName) {
  const key = normExName(exName);
  const ex = ((rec && rec.exercises) || []).find(e => normExName(e.name) === key);
  return recSetsForEntry(rec, ex);
}
...
function allSetsFor(rec, exName) {
  return recSetsFor(rec, exName).filter(s => s && entryMode(s));
}
...
function setLogsFor(rec, exName) {
  return recSetsFor(rec, exName).filter(s => s && s.weight > 0);
}
...
function holdSecondsLogsFor(rec, exName) {
  return recSetsFor(rec, exName).filter(s => s && s.seconds != null);
}
...
// v0.9.21d — PREV NEVER READS A DELOAD RECORD. The week after a deload, every
// PREV showed the deload loads, so the working weight looked like it had dropped
// for good. A deload week is a distinct thing: its records are stamped `deload`
// and skipped here, so PREV always means "last NORMAL session". If the only
// history for an exercise IS deload records (a lift introduced that week), it
// falls back to them rather than show nothing.
function getMostUsedSetValue(exName, _includeDeload) {
  ...
  for (let i = 0; i < state.history.length && sessionsSeen < 10; i++) {
    if (!_includeDeload && state.history[i] && state.history[i].deload) { skippedDeload = true; continue; }
  ...
  if (!allSets.length) return (skippedDeload && !_includeDeload) ? getMostUsedSetValue(exName, true) : null;
...
function renderWorkoutBlocks(day) {
  ...
  day.blocks.forEach(block => {
    ...
    if (block.type==='cardio') {
      ...
      block.exercises.forEach(ex => {
        ...
        // explicit progression target — showed nothing at all.
        // v0.9.21c: `hint` stays the WEIGHTED shape, but hold mode no longer goes
        // without a previous value — it gets its own, in its own unit. The old note
        // here read "the deload comparison below reads hint.weight ... the other two
        // modes deliberately leave `hint` null", and that is precisely why the
        // eccentric pull-up was the ONE lift whose chips stayed unhighlighted on a
        // deload week: the comparison could only think in kilograms. It now compares
        // seconds to seconds. `prevHold` is the LAST hold set rather than the most-used
        // one — the same source the on-screen hint already displays, so the chip and
        // the hint can never disagree about what "last time" was.
        const hint = (isBodyweight || isHoldSeconds) ? null : getMostUsedSetValue(ex.name);
        const prevHold = isHoldSeconds ? lastHoldSetFor(ex.name) : null;
        let hintText = hint ? `${hint.weight}kg × ${hint.reps}` : '';
        if (isHoldSeconds) {
          // Same wording as the set chip (holdLabel): '15s × 2', never a bare '15s'.
          hintText = prevHold ? holdLabel(prevHold) : '';
        ...
        if (block.type!=='warmup') {
          for (let s=0;s<ex.sets;s++) {
            ...
            // v0.9.21c: now in the exercise's OWN unit. Holds were excluded outright,
            // so on a deload week every weighted lift went blue and the eccentric
            // pull-up alone stayed unmarked — the one lift whose deload target Albin
            // had hit exactly. Bodyweight is still excluded, deliberately: a deload
            // there is fewer reps, and reps are also how a hold's volume is written,
            // so the two are not the same comparison and guessing at one would be a
            // second bug rather than a fix (#15).
            const dlPrev = isHoldSeconds ? (prevHold ? prevHold.seconds : null)
                                         : (hint ? hint.weight : null);
            const dlCur  = logged ? (isHoldSeconds ? logged.seconds : logged.weight) : null;
            const isDeloadLower = logged && !isBodyweight && isDeloadActive()
              && dlPrev != null && dlCur != null && dlCur < dlPrev;
  ...
  // v0.9.21 (#56): the manual way out. The prompt fires at most once per trigger
  // per session, so declining it — 'STILL LIFTING' — used to leave NO route to
  // closing the strength half at all. An escape hatch must not be reachable only
  // through the thing you are escaping.
  appendStrengthDoneRow(day, container);
...
// v0.9.21 ── STRENGTH DONE ROW ───────────────────────────────────────────────
// Sits between the last strength block and CARDIO, which is where the strength
// half actually ends. PRESENCE-DRIVEN like the CARDIO section: a day with no main
// or secondary block has no strength half to close, so no row is drawn — that
// rules out stretch days, rest days and the Active Day without naming any of them
// (#15: the correction names its scope).
//
// Two states, and the row VALIDATES ON SHOW rather than only on tap (#13): once
// the half is closed it stops being a button and becomes the recorded figure, so
// the number is visible without having to trust that the tap worked.
function dayHasStrengthBlocks(day) {
  return (day && day.blocks || []).some(b => b && (b.type === 'main' || b.type === 'secondary'));
}
function appendStrengthDoneRow(day, container) {
  if (!day || !container || !dayHasStrengthBlocks(day)) return;
  if (state._editingHistoryId) return;      // never while editing a past session
  const sess = getSession(day.id);
  const el = document.createElement('div');
  el.className = 'strength-done-row';
  if (sess.strengthSecs != null) {
    el.innerHTML = `<div class="strength-done-recorded">
        <span class="sd-tick">✓</span>
        <span class="sd-label">STRENGTH TIME</span>
        <span class="sd-value">${fmtClock(+sess.strengthSecs)}</span>
      </div>
      <div class="sd-hint">Restart the clock to reopen it.</div>`;
  } else {
    el.innerHTML = `<button class="strength-done-btn" onclick="strengthDoneTapped('${day.id}')">
        ✓ STRENGTH SESSION DONE
      </button>
      <div class="sd-hint">Stops the clock here. Cardio minutes come from their own log.</div>`;
  }
  container.appendChild(el);
}
// Same dialog the automatic prompt uses, so the time is shown before it is
// committed and there is exactly one path that writes the stamp. The lead line
// differs because the automatic one can truthfully say every set is logged and
// this one cannot — it is reached by choice, at any point.
function strengthDoneTapped(dayId) {
  confirmCloseStrengthSession(dayId, 'Finishing your strength session.');
}
...
function cardioItemsForDay(day) {
  ...
  // v0.9.20: 'cardio_day' (D6, Active Day) joins the full-pool branch. It used to
  // fall through to the line below, and because D6 declares `commute: false` that
  // returned an EMPTY array — the gym's equipment was unreachable on the one day
  // built around using it. The last line now means what it reads as: days that are
  // not training/stretch/cardio get the commute row only.
  if (day.type === 'training' || day.type === 'stretch' || day.type === 'cardio_day') {
...
// v0.9.21 — ONE PLACE THE STRENGTH HALF IS CLOSED. Two callers now reach it: the
// STRENGTH SESSION OVER? prompt, and the manual button between the last strength
// block and CARDIO. Same rule as buildHistoryRecord() — a second assembly point is
// how the three save paths drifted apart, so there isn't one.
function closeStrengthSession(dayId, secs) {
  const s = getSession(dayId);
  s.timerAutoStopped = true;
  pauseTimer();
  // STAMPED, NOT RE-DERIVED. Until now the number survived only as paused clock
  // state and was read back by getTimerSecs() at finish, so the one witness to a
  // closed strength half was the clock itself. Review does not restore
  // timerAutoStopped (doneDayAction() copies nine session fields and not that one),
  // and load() clears a clock that is not plausibly from today — so reopening a
  // finished day re-armed the whole thing and left the stored 50:15 defensible only
  // by durationWouldBeOverwritten()'s prompt. Once Albin has SAID the strength half
  // is over, that is a fact about the session and is stored as one — the same
  // load-stamp-over-derive rule the bodyweight loads follow.
  s.strengthSecs = secs;
  saveSession();
  showTimerAutoToast('STRENGTH TIME: ' + fmtClock(secs));
  const day = state.workoutDay;
  if (day && day.id === dayId) { renderWorkoutBlocks(day); updateProgress(); }
}
function fmtClock(secs) { return pad2(Math.floor(secs/60)) + ':' + pad2(secs%60); }
// Shared dialog. `lead` is the first line — it differs by how we got here, and
// saying the wrong one ('every set is logged') when it isn't true would be a typed
// claim about the athlete's own session (#12).
function confirmCloseStrengthSession(dayId, lead) {
  const secs = getTimerSecs();
  const stamp = fmtClock(secs);
  appConfirm('STRENGTH SESSION OVER?',
    lead + ' Your strength time will be recorded as ' + stamp + '. ' +
    'Cardio minutes come from their own log, so nothing after this is lost.',
    function () { closeStrengthSession(dayId, secs); },
    'YES, DONE', 'STILL LIFTING');
}
function maybeOfferSessionStop(trigger) {
  ...
  saveSession();
  // v0.9.21 — ASK ABOUT THE TRAINING, NOT THE MECHANISM. The old copy asked
  // whether to stop a clock, which is an operation on an internal; the answer the
  // athlete actually has is whether he is done lifting. The engine derives the
  // clock behaviour from that. pauseTimer() still runs underneath.
  confirmCloseStrengthSession(day.id, (trigger === 'sets')
    ? 'Every prescribed set is logged.'
    : 'Every main and secondary exercise is ticked.');
}
...
function resumeTimer() {
  if (state.timerRunning) return;
  // v0.9.21: restarting the clock after declaring the strength half over
  // contradicts the declaration. The later explicit act wins, so the stamp is
  // dropped and the clock becomes the witness again. `timerAutoStopped` is left
  // alone — it only gates the auto-START, and the athlete has just started it by
  // hand, so clearing it would change nothing except re-arming a prompt that
  // `stopPrompt` already holds shut.
  const day = state.workoutDay;
  if (day && state.sessions[day.id]) delete state.sessions[day.id].strengthSecs;
  startTimerTick();
}
// ── APP DIALOG (v0.9.6) — replaces native confirm/alert ─────────
// v0.9.19: onCancel added as an optional SIXTH parameter. Needed because the E15
// duration prompt has two real outcomes — replace the stored time, or keep it — and
// both must still save the session. A dialog whose cancel silently discards an edit
// would trade one quiet data loss for another. Existing callers pass five arguments
// or fewer and are unaffected.
function appConfirm(title, msg, onConfirm, confirmLabel, cancelLabel, onCancel) {
  ...
  mEl.textContent = msg;
  bEl.innerHTML = `
    <button class="modal-btn cancel" id="app-dialog-cancel">${cancelLabel||'CANCEL'}</button>
  ...
  document.getElementById('app-dialog-cancel').onclick =
    () => { closeAppDialog(); if (onCancel) onCancel(); };
  el.style.display = 'flex';
}
...
function autoTickRestDay() {
  ...
  // v0.9.19: NO HISTORY RECORD IS WRITTEN. This was the fourth of five record
  // construction sites; it now writes none. A rest day is a day nothing happened
  // on, and a record saying so is an empty row in History, a dot on the calendar
  // for a day not trained, and — until the rewrite above — a load-bearing input to
  // the streak. The board tick survives without it: the Home day cards read
  // `state.weekSessions`, never history (v0.9.11, #3).
  // `prev` and `recDate` are still resolved above, because the future-guard needs
  // the date and a rest day CAN legitimately carry a real record (a logged commute
  // on a day whose PROGRAM entry does not opt out of it).
...
function autoSaveSessionToHistory(dayId) {
  ...
  // v0.9.18 (Albin's call): a FINISHED record is never touched by an autosave.
  // The Home card's "Review" action reopens a completed day WITHOUT edit mode, so
  // this function did not bail — it rebuilt the finished record from the live
  // session and, until the builder above, rebuilt it INCOMPLETE: cardio, commute,
  // active-day and yoga logs all dropped, and inProgress flipped back to true on a
  // session that was already done. Reproduced on v0.9.17b before this changed.
  // The rule is the one v0.9.16 already applied to the edit path: a reopened
  // session commits on FINISH or not at all. Refusing REMOVES the failure mode
  // rather than depending forever on the builder staying complete (#38b), and
  // makes Review behave the same as Resume & Edit (#34).
  // "Finished" is read from a declared field, not inferred (#42): autosave is the
  // only writer of inProgress, so its absence means the record was finished.
  if (_prev && !_prev.inProgress) return;
  ...
  // measured — the per-part keep inside buildHistoryRecord() handles that.
  const record = buildHistoryRecord(day, sess, {
    prev: _prev,
  ...
  // v0.9.19 (Albin, 27 Aug): an EMPTY in-progress record is kept only on a TRAINING
  // day. Tapping into a rest, stretch or active day to look at it is not an
  // abandoned workout, and leaving a record behind is what put UNFINISHED chips on
  // seven passive days in the live history.
  // Note the test is on CONTENT, not day type alone: a stretch day carrying a yoga
  // log, or an active day with an activity logged, is real and survives regardless
  // of never being finished. Only the empty ones go.
  if (!recordHasContent(record) && recordDayType(record) !== 'training') {
    if (existing >= 0) { state.history.splice(existing, 1); save(); }
    return;
  }
...
// v0.9.21 — THE ACTIVE-DAY FILL SITS HERE, AND THE POSITION IS THE WHOLE POINT.
// Putting it in sessionDurationParts() would make dp.strength non-zero on an
// Active Day, which stands the keep-guard below down: reviewing 29-08 (clock
// reset to 0, activeLog holding 60 min) would then write 3600 over a stored
// 7031. That is the E15 fault this guard exists for. Order is therefore:
//   1. live clock ran            → it wins
//   2. clock dead, value stored  → the stored measurement wins (existing guard)
//   3. still zero, minutes logged → fill from the declared activity minutes
// A filled value is MARKED ESTIMATED. It is a self-report, and for a preset it is
// literally the program's own '~1h' — durationLabel() renders '~60m', so no
// derived number is ever mistaken for a measured one (#12).
function keptDurationParts(dp, prev) {
  ...
  const pEstimate = !!(prev && prev.durationEstimated);
  // v0.9.21b — THE GUARD PROTECTS MEASUREMENTS, NOT DERIVATIONS. The keep rule
  // exists so a dead clock cannot wipe a real recorded time. It was also shielding
  // a value this release had DERIVED from the activity log, which made the walk
  // preset look stuck: finish a Short walk (60m stored, estimated), reopen, switch
  // to Long, save — and the stored 3600 won over the fresh 6300, so History still
  // read ~60m. A derived value must re-derive when its input changes; a measured
  // one must never be overwritten. `durationEstimated` is exactly that distinction
  // and it is already stamped on the record, so it is read, not inferred (#11).
  //
  // The v0.9.15 backfill estimates are untouched by this: they only exist on
  // training days, where `active` is 0 by scope, so the extra clause never fires.
  const keep     = (!dp.strength && pStrength > 0 && !(pEstimate && active > 0));
  let strength   = keep ? pStrength : dp.strength;
  const cardio   = dp.cardio;
  // a kept value keeps its provenance: an estimate stays an estimate
  let estimated  = keep ? pEstimate : false;
  if (!strength && active > 0) { strength = active; estimated = true; }
  return {
    strength, cardio,
...
// ── HISTORY RECORD BUILDER (v0.9.18, roadmap B5) ──────────────────────────
// ONE place a history record is assembled. It used to be three: finishWorkout()'s
// non-edit branch, its edit branch, and autoSaveSessionToHistory(). FOUR data-loss
// bugs have had the identical cause — a field present in two of the three and
// absent from the third:
//   v0.9.15  duration guard      missing from the non-edit branch  → sessions zeroed
//   v0.9.16  per-part keep       all three needed revisiting       → cardio edits ate strength time
//   v0.9.17b structured cardio   missing from the non-edit branch  → cardio lost on edit
//   v0.9.18  cardio + activeLog  missing from autosave AND the edit branch
//                                                                  → cardio wiped by a Review,
//                                                                    and edited cardio stored stale
// dev_rules #43 said to grep the field name and check all three. The rule held;
// the checking did not. So the three paths no longer differ in what they build —
// only in the four values that genuinely vary, all passed IN:
//   id · date · weekKey · inProgress
// Everything else is derived here, once, for everybody.

// The exercises array — every entry a session can produce, in one pass:
// strength/stretch entries, cardio pool items, the yoga summary, active-day
// activities. autoSaveSessionToHistory() built only the first of those four,
// which is why a mid-session autosave used to drop the cardio lines outright.
function buildSessionExercises(day, sess) {
  const exercises = [];
  if (day.type === 'rest') {
    // nothing to enumerate
  } else if (day.type === 'stretch') {
    stretchMoves(day).forEach(m => {
      // v0.9.18: sets comes from the move's own PROGRAM entry. The history
      // editor's numSets reads `ex.sets || progEx.sets || 1` — writing a flat 1
      // would CAP a multi-set stretch move at one box (#12: a value a human is
      // told to trust is derived, never typed). Albin's D5 declares sets:1 today;
      // Emily's program is content-owned and need not.
      exercises.push({id:m.id, name:m.name, checked:!!sess.workoutChecks[m.id],
        sets: m.sets || 1, weight:''});
    });
  } else {
    day.blocks.forEach(b => b.exercises.forEach(ex => {
      ...
      // v0.9.20 (B4): no `setLogs` here. The sets live once, in record.setLogs,
      // cloned by buildHistoryRecord() from the same sess.setLogs this summary is
      // built from. `weight` stays — it is a rendered SUMMARY, not the data (#45),
      // and History lists it without opening the record.
  ...
  cardioItemsForDay(day).forEach(item => {
    ...
    exercises.push({id:item.id, name:item.name, checked:true, type:'cardio',
      weight: log ? formatCardioLog(log) : ''});
  });
  // v0.9.9 (#5): yoga session log — pushed as a single summary entry.
  if (day.type === 'stretch' && day.yogaLog && sess.yogaSessionLog &&
      (sess.yogaSessionLog.duration || sess.yogaSessionLog.style)) {
    const yl = sess.yogaSessionLog;
    const parts = [yl.duration?`${yl.duration} min`:null, yl.style||null].filter(Boolean);
    exercises.push({id:'yoga_session', name:'Yoga', checked:true,
      weight: parts.join(' · ')});
  }
  // v0.9.9 (#7): active-day activities (Saturday) — each picked activity + minutes.
...
// A session with nothing in it — for callers that must build a record without one.
function emptySessionShape() {
  return { workoutChecks:{}, setLogs:{}, cardioChecks:{}, cardioSelected:{},
           cardioLogs:{}, commuteChecked:false, commuteLog:null, activeLog:{},
           yogaSessionLog:{duration:null, style:null} };
}
// opts: { prev, id, date, weekKey, inProgress, durationParts }
//   prev       — the record being written over, or null. Feeds the per-part
//                duration keep; a dead clock never overwrites a measured value.
//   id/date/weekKey — an edit keeps the record's OWN date and week; a fresh save
//                takes today's. Passed in, never re-derived here (#12).
//   inProgress — declared, not inferred (#42). true only for an autosave.
function buildHistoryRecord(day, sess, opts) {
  const o    = opts || {};
  const prev = o.prev || null;
  // Callers with no clock of their own pass durationParts explicitly rather than
  // letting it be derived from the live timer (see autoTickRestDay).
  const _dp  = o.durationParts || sessionDurationParts(day, sess);
  const _kp  = keptDurationParts(_dp, prev);
  const rec  = {
    id:        o.id,
    dayId:     day.id,
    dayName:   day.name,
    // v0.9.19: the record declares its OWN day type instead of leaving readers to
    // resolve dayId against whatever PROGRAM says today (#42). Albin's week has been
    // reshuffled at least once — the live history holds a D6 named 'Legs + Glutes'
    // and two D4s named 'Rest' — so dayId alone answers with today's mapping and
    // gets those backwards. Stamped at build time, like `load` (#12 in spirit:
    // a value a reader depends on is recorded when it is known, not re-derived).
    // Only one edit because buildHistoryRecord() is the sole assembly point (#43,
    // superseded by B5); grepped `dayName:` to prove it before adding this.
    dayType:   day.type || null,
    date:      o.date,
    weekKey:   o.weekKey,
    dateStr:   recordDateStr(o.date),
    // v0.9.21d: DECLARED on the record, for ITS week (o.weekKey, not the clock).
    // Read by History and the calendar to mark the week, and by every PREV reader
    // to skip it — a deload week and a normal week are two distinct things and
    // the record now says which it is. One edit, sole assembly point (B5).
    deload:    isDeloadWeek(o.weekKey),
    duration:  _kp.total,
    durationStr: formatDuration(_kp.total),
    ...
    exercises: buildSessionExercises(day, sess),
  };
  // Only ever present on an autosave. A finished record must not carry the key
  // at all — isSessionTouched()/the Home cards read its absence as "finished".
  if (o.inProgress) rec.inProgress = true;
  return rec;
}

// ── FINISH WORKOUT ────────────────────────────
// v0.9.19 — E15. keptDurationParts() only defends against a DEAD clock: if the live
// clock reads 0 it keeps the stored strength time. It does nothing when a clock
// actually ran, so starting a new timer during a review REPLACES a real measurement
// with a short one, silently and by design. Two records in the live history were
// lost this way — a 26-set leg session stored as 5m 00s, a full shoulder session as
// 1m 08s — and nothing on screen said it had happened.
// Scoped (#15) to reviewing a FINISHED record. Resuming an in-progress session is
// normal flow, its stored duration is a partial autosave rather than a measurement,
// and prompting there would fire on every ordinary resume.
function durationWouldBeOverwritten(prev, sess, day) {
  if (!prev || prev.inProgress) return null;
  const stored = prev.durationStrength != null ? prev.durationStrength : (prev.duration || 0);
  if (!(stored > 0)) return null;
  const live = sessionDurationParts(day, sess).strength;
  if (!(live > 0) || live === stored) return null;
  return { stored, live };
}
...
function finishWorkout(opts) {
  ...
  // Ask BEFORE anything is built or written. Re-entered with the flag on confirm,
  // so the guard is answered once and the save path below is untouched.
  if (!(opts && opts.durationConfirmed)) {
    const _i = editMode
    ...
    const clash = durationWouldBeOverwritten(_i >= 0 ? state.history[_i] : null, sess, day);
    if (clash) {
      // KEEPING the stored measurement is the PRIMARY action, so it takes the green
      // confirm slot. The first cut had it the other way round and the screen made
      // the case plainly: a 9-second clock rendered as the recommended choice next to
      // a greyed-out 70-minute session. The destructive option must never be the one
      // the styling argues for.
      appConfirm('SESSION TIME',
        `This session is stored as ${formatDuration(clash.stored)}. The timer you just ` +
        `ran reads ${formatDuration(clash.live)}. Which should be saved?`,
        () => finishWorkout({ durationConfirmed: true, keepStoredStrength: clash.stored }),
        'KEEP ' + formatDuration(clash.stored).toUpperCase(),
        'USE '  + formatDuration(clash.live).toUpperCase(),
        () => finishWorkout({ durationConfirmed: true }));
      return;
    }
  }

  // v0.9.18 (roadmap B5): both branches build the record the same way, through
  // buildHistoryRecord(). What genuinely differs between them — which record id,
  // which date, which week — is passed IN. Four data-loss bugs came from these
  // two branches and the autosave drifting apart; they can no longer drift,
  // because there is only one assembly site left.
  const _editIdx = editMode
    ? state.history.findIndex(r => r.id === state._editingHistoryId)
    : state.history.findIndex(h => h.dayId===day.id && h.weekKey===state.currentWeekKey);
  const _prev = _editIdx >= 0 ? state.history[_editIdx] : null;

  // v0.9.19 E15: when KEEP is chosen, the stored strength time is passed in as an
  // explicit override rather than left to keptDurationParts(), which only defends
  // against a clock reading zero and would otherwise let the live value through.
  const _keepDP = (opts && opts.keepStoredStrength != null)
    ? (() => { const c = sessionDurationParts(day, sess).cardio || 0;
               return { strength: opts.keepStoredStrength, cardio: c,
                        total: opts.keepStoredStrength + c, clamped: false }; })()
    : null;
  ...
  if (editMode) {
    if (_editIdx >= 0) {
      ...
      // from a dead clock and wrote 0 over a real measurement. The per-part keep
      // inside the builder is what stops that; _prev is what it reads.
      // The edited record keeps its OWN id, date and week — an edit must never
      // re-date a session to today (#45: a record's date is stored, not re-derived).
      const record = buildHistoryRecord(day, sess, {
        prev:    _prev,
        id:      _prev.id,
        date:    _prev.date,
        weekKey: _prev.weekKey,
        durationParts: _keepDP || undefined,   // v0.9.19 E15
        // no inProgress: saving an edit finishes it
      });
      // v0.9.18: this branch used to spread the old record and overwrite a
      // hand-listed subset of its fields. cardioLogs was not in that subset, so
      // editing the treadmill from 20 to 35 min updated the DISPLAYED string while
      // the stored numbers stayed at 20 — display and data disagreeing, which is
      // exactly what #14 forbids. Reproduced on v0.9.17b. A full rebuild has no
      // subset to forget.
      state.history[_editIdx] = record;
      duration = record.duration;
    }
    state._editingHistoryId = null;
  } else {
    const existing = _editIdx;
    const recDate  = recordDateFor(day.id, state.currentWeekKey, _prev);
    const record   = buildHistoryRecord(day, sess, {
      prev:    _prev,
      id:      existing >= 0 ? _prev.id : Date.now(),
      date:    recDate,
      weekKey: state.currentWeekKey,
      durationParts: _keepDP || undefined,   // v0.9.19 E15 — the Review route lands here
    });
    duration = record.duration;
    // The week's own copy of the session — separate from the history record, and
    // what the Home card's Review action restores from.
    state.weekSessions[day.id] = {
      ...
      activeLog:      JSON.parse(JSON.stringify(sess.activeLog || {})),
      yogaSessionLog: JSON.parse(JSON.stringify(sess.yogaSessionLog || {duration:null,style:null})),
      exercises: record.exercises, duration
    };
    // v0.9.19: A FINISHED DAY WITH NOTHING LOGGED COUNTS AS NOTHING DONE, and is
    // not written. Note the second half — an existing record is REMOVED, not just
    // left unwritten. Touching one control autosaves an in-progress record; if
    // FINISH then found nothing and merely declined to write, that record would
    // survive and the day would read UNFINISHED forever despite having been
    // finished. Pressing FINISH is a declared act, not abandonment.
    // The weekSessions tick above is deliberately kept: the board reflects that the
    // day was closed out, exactly as it does for a rest day with no record.
    // SCOPE (#15): this gate is on the NON-EDIT branch only. The edit branch above
    // is untouched — emptying a record you deliberately opened is curation, and
    // deleting it out from under SAVE EDITS would be a different, unrequested
    // behaviour.
    if (!recordHasContent(record)) {
      if (existing >= 0) state.history.splice(existing, 1);
    }
    else if (existing>=0) state.history[existing]=record; else state.history.unshift(record);
  ...
  // v0.9.18: the AI wrap-up summary is built from the record that was just
  // written, not from a second traversal of the session — one implementation per
  // derived value (#22). An edit that found no record produces an empty list.
  const _finExercises = (editMode ? (state.history[_editIdx] || {}).exercises
                                  : (state.history.find(h => h.dayId===day.id && h.weekKey===state.currentWeekKey) || {}).exercises) || [];
  const setCount = _finExercises.reduce((n,e)=>n+(e.setLogs||[]).filter(s=>s&&s.reps!=null).length,0);
  state._lastFinished = { dayName: finishedDay.name, dayType: finishedDay.type,
    durationStr: formatDuration(duration), setCount, exercises: _finExercises };
...
function executeWeekReset() {
  ...
  // v0.9.21d: load() no longer pre-advances the key while the rollover is pending
  // (the board keeps last week's key so its sessions keep last week's dates), so
  // accepting the rollover is where the key moves to the real week.
...
// Pure, so it can be asserted: the key the board moves to when the user saves and
// moves on. +1 only from a board that IS the real week; a board that fell behind
// moves to the real week, never to real-week-plus-one.
function nextBoardWeekKey(currentKey, realWk) {
  const parts = String(currentKey || realWk).split('-W');
  const yr = parseInt(parts[0]), wk = parseInt(parts[1]);
  const newWk = wk >= 52 ? 1 : wk + 1;
  const newYr = wk >= 52 ? yr + 1 : yr;
  const plusOne = `${newYr}-W${String(newWk).padStart(2,'0')}`;
  return (currentKey < realWk) ? realWk : plusOne;
}
function saveAndNextWeek() {
  appConfirm(
    ...
    () => {
      ...
      // v0.9.21d: +1 is only right when the board IS the real week. If the board was
      // BEHIND (the rollover was declined and last week's sessions were being finished
      // off), "next week" means the REAL current week, not last-week-plus-one and
      // certainly not real-week-plus-one. The archive above already ran under the
      // board's own key, so the sessions keep last week's dates.
      state.currentWeekKey = nextBoardWeekKey(state.currentWeekKey, getWeekKey());
      resetTimer();
      save();
      buildHomeDayCards();
      document.getElementById('header-progress-wrap').classList.remove('visible');
...
function executeWeekResetFromSettings() {
  ...
  state.sessions          = {};
  state._pendingWeekReset = false;
  state.currentWeekKey    = getWeekKey();   // v0.9.21d — same reason as executeWeekReset()
  state._lastWeekInprog   = null;
  state.workoutDay        = null;
  resetTimer();
  save();
...
// ── HISTORY ──────────────────────────────────
// v0.9.21d — 'Mon 7th Sept', DERIVED from the ISO date every time it is drawn.
// History used to render h.dateStr, a display string typed onto the record at
// write time. The calendar rendered h.date. When a date was edited by hand only
// `date` moved, so the two screens disagreed for the same record. A typed copy
// of a derivable value is a drift waiting to happen (#12, #45) — this reads the
// one true field and formats it, and dateStr is kept only for older consumers.
function fmtHistoryDate(iso) {
  ...
  const suf = (n % 10 === 1 && n !== 11) ? 'st' : (n % 10 === 2 && n !== 12) ? 'nd'
            : (n % 10 === 3 && n !== 13) ? 'rd' : 'th';
  const mon = ['Jan','Feb','Mar','Apr','May','June','July','Aug','Sept','Oct','Nov','Dec'][d.getMonth()];
  const dow = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d.getDay()];
  return `${dow} ${n}${suf} ${mon}`;
}
// Read-only detail panel: the same canonical set reader the editor uses
// (recSetsForEntry), never the per-exercise legacy copy. Nothing here is tappable —
// Modify is the editor, this is the receipt.
function historyDetailHtml(h) {
  const rows = [];
  (h.exercises || []).forEach(ex => {
    if (!ex) return;
    const progEx = progExInDay(h.dayId, ex.id);
    const progBlock = progBlockInDay(h.dayId, ex.id);
    const isWarmup = !!(progBlock && progBlock.type === 'warmup');
    const isCardio = !ex.orphan && (ex.type==='cardio' || ex.type==='check' || !!cardioPoolItem(ex.id));
    const sets = recSetsForEntry(h, ex);
    let vals = '';
    if (isCardio) {
      const cl = (h.cardioLogs||{})[ex.id];
      vals = cl ? formatCardioLog(cl) : (ex.weight || '');
    } else if (isWarmup) {
      vals = ex.checked ? '✓' : '';
    } else if (sets.length) {
      const mode = histModeFor(ex, progEx, sets[0]);
      vals = sets.map(s => !s ? '·'
        : mode === 'hold'       ? holdLabel(s)
        : mode === 'bodyweight' ? `${s.reps} reps`
        : `${s.weight}kg × ${s.reps}`).join(' · ');
    } else if (ex.weight) {
      vals = String(ex.weight);          // Active Day entries carry their summary here
    }
    if (!vals && !ex.checked) return;
    rows.push(`<div class="hd-row${ex.orphan ? ' hd-orphan' : ''}">
        <span class="hd-name">${esc(ex.name || '')}</span>
        <span class="hd-vals">${esc(vals)}</span>
      </div>`);
  });
  return rows.length ? rows.join('') : '<div class="hd-row hd-empty">Nothing logged.</div>';
}
function toggleHistoryCard(id) {
  const card = document.querySelector(`.history-entry[data-id="${id}"]`);
  if (!card) return;
  card.classList.toggle('open');
}
function collapseAllHistory() {
  document.querySelectorAll('.history-entry.open').forEach(c => c.classList.remove('open'));
}
function renderHistory() {
  ...
  bar.innerHTML = `<button class="hc-collapse-all" onclick="collapseAllHistory()">▴ Collapse all</button>`;
  c.appendChild(bar);
  state.history.forEach(h=>{
    ...
    // v0.9.19: read straight off the declared field (#42) — autoSaveSessionToHistory()
    // is the only writer of inProgress, so its presence IS "opened and never finished".
    // No new state, no heuristic, and nothing inferred from the record's shape.
    const unfinished = !!h.inProgress;
    // v0.9.21d: `deload` is stamped on the record by buildHistoryRecord() and by the
    // one-time retro pass over deloadDecision.accepted[] — read, never inferred.
    const deload = !!h.deload;
    card.innerHTML=`
      <div class="hc-top" onclick="toggleHistoryCard(${h.id})">
        <span class="hc-day">${h.dayId} — ${h.dayName}${deload ? ' <span class="hc-deload">DELOAD</span>' : ''}</span>
        <span class="hc-date">${fmtHistoryDate(h.date)}</span>
        <span class="hc-chevron" aria-hidden="true">▾</span>
      </div>
      ${unfinished ? '<div class="hc-unfinished">UNFINISHED</div>' : ''}
      <div class="hc-focus">${done} exercises · ${sets} sets · ${durationLabel(h)}</div>
      <div class="hc-detail">${historyDetailHtml(h)}</div>
...
function renderModifyBody(record) {
  ...
  (record.exercises||[]).forEach((ex,idx)=>{
    ...
    // v0.9.20: an orphan declares itself (#42) and is never cardio — its id came
    // from a retired PROGRAM entry and could collide with a cardio pool id.
    const isCardio = !ex.orphan && (ex.type==='cardio' || ex.type==='check' || !!cardioPoolItem(ex.id)
...
function saveModify() {
  ...
  if(r) {
    // Save date change
    // v0.9.21d: a date is not one field. weekKey is derived from it (every week-
    // grouped lookup, the streak and the milestones read weekKey, never date), and
    // dateStr was a typed copy that History rendered while the calendar rendered
    // date — Albin re-dated three records by hand and got one answer on the
    // calendar and another in History. All three now move together (#12, #45).
    const dateInput = document.getElementById('modify-date-input');
    if (dateInput && dateInput.value && dateInput.value !== r.date) {
      r.date    = dateInput.value;
      r.weekKey = getWeekKey(new Date(r.date + 'T12:00:00'));
      r.dateStr = recordDateStr(r.date);
    }
...
function _latestHistorySets(exName, pick, _includeDeload) {
  const sorted = (state.history || []).filter(h => h && h.date).slice()
    .sort((a, b) => (b.date || '').localeCompare(a.date || ''));
  let skippedDeload = false;
  for (const rec of sorted) {
    if (!_includeDeload && rec.deload) { skippedDeload = true; continue; }   // v0.9.21d — see getMostUsedSetValue()
    const sets = pick(rec, exName);
    if (sets.length) return sets;
  }
  return (skippedDeload && !_includeDeload) ? _latestHistorySets(exName, pick, true) : null;
}
...
function bodyweightLogsFor(rec, exName) {
  return recSetsFor(rec, exName)
    .filter(s => s && s.reps != null && s.seconds == null && !(s.weight > 0));
}
...
function renderCalendar() {
  ...
  // v0.9.19: the map now carries whether each record was FINISHED, not just its
  // dayId. Before this, an abandoned day got the same green training dot as a
  // completed session — History said UNFINISHED, the streak excluded it, and the
  // calendar said you trained. Three views, two answers (#42: read the declared
  // field, don't infer from the record's shape).
  const sessionDates={};
  state.history.forEach(h=>{ sessionDates[h.date]=sessionDates[h.date]||[];
    sessionDates[h.date].push({ dayId:h.dayId, done:!h.inProgress, deload:!!h.deload }); });
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    let dotColor='';
    if(days.length){
      // A finished record on the date wins the colour. Grey is only reached when
      // EVERY record for that date is still in progress, so a day where one session
      // was completed and another abandoned still reads as trained.
      const done=days.filter(x=>x.done);
      if(done.length){
        const types=done.map(x=>{const p=PROGRAM.find(p=>p.id===x.dayId);return p?p.type:'';});
        ...
        dotColor='var(--cal-dot-idle)';   // started, never finished
      }
    }
    // v0.9.21d: a deload day keeps its type colour on the dot and gets a ring in
    // the deload blue, so the week reads as deload at a glance without hiding
    // what kind of day it was. Read from the stamped field, never inferred.
    const isDeload = days.some(x=>x.deload);
    if (isDeload) cell.classList.add('cal-deload');
...
function renderStats() {
  ...
  state.history.forEach(h=>{ (h.exercises||[]).forEach(ex=>{
    // v0.9.20 (B4): through the resolver, so a migrated record still qualifies.
    if(recSetsForEntry(h,ex).some(s=>s&&entryMode(s))) names.add(ex.name);
  }); });
...
// v0.9.19. Prefer the type the record STAMPED on itself; fall back to resolving its
// dayId against the current PROGRAM only for records written before v0.9.19.
// A lookup can miss, an inference must answer (#38) — the stamp can be absent and
// says so, whereas dayTypeForId() always returns something, right or wrong.
// KNOWN LIMIT, not fixed here: legacy records still resolve by today's mapping, so
// the four pre-v0.9.19 records whose dayId and dayName disagree are still judged by
// dayId. Name-matching them would be inferring a declared field from a display
// string, which is exactly what #11 forbids; those are better deleted by hand.
function recordDayType(rec){
  if (!rec) return null;
  if (rec.dayType) return rec.dayType;
  return dayTypeForId(rec.dayId);
}
...
function recordHasContent(rec){
  ...
  // v0.9.19: CARDIO WAS MISSING FROM THIS TEST. buildSessionExercises() pushes
  // every cardio entry with `setLogs: []`, so setCount() cannot see one, and
  // `checks` is workoutChecks only — cardioSelected / cardioChecks / commute were
  // never consulted. A day where the ONLY thing logged was the treadmill, or the
  // bike commute, therefore read as EMPTY. That was survivable while the single
  // consumer was weekTypeCounts(), where the cost is a missed milestone tier. It
  // is not survivable now that this function decides whether a record is WRITTEN
  // and whether an existing one is DELETED (#19: listing a consumer is not
  // reading it).
  if (rec.cardioSelected && Object.values(rec.cardioSelected).some(Boolean)) return true;
  if (rec.cardioChecks   && Object.values(rec.cardioChecks).some(Boolean))   return true;
  if (rec.cardioLogs     && Object.keys(rec.cardioLogs).length)              return true;
  if (rec.commuteChecked) return true;
  if (rec.commuteLog && Object.keys(rec.commuteLog).length) return true;
  if (rec.yogaSessionLog && (rec.yogaSessionLog.duration || rec.yogaSessionLog.style)) return true;
  if (activeLogHasContent(rec)) return true;
  return false;
}
// v0.9.21 — 'Rest / Nothing' IS NOT CONTENT. The chip declares `rest: true` in the
// program, so this reads a declared flag and never a label (#11). An Active Day
// where the only tap was 'Rest / Nothing' was writing a record — 18-07 and 25-07
// both sat in History as '0 exercises · 0 sets · —' — because the old test was
// simply "activeLog has a key", and {nothing:{mins:''}} has one.
//
// FAILS SAFE BY CONSTRUCTION (#38): the day lookup can MISS — a reshuffled or
// renamed day, or a program that no longer declares the chip — and a miss yields
// an empty rest set, so every entry counts as content and the behaviour is exactly
// what it was before this release. It never infers a rest day from a name.
function activeLogHasContent(rec){
  const al = rec && rec.activeLog;
  if (!al || typeof al !== 'object') return false;
  const ids = Object.keys(al);
  if (!ids.length) return false;
  const day = (typeof PROGRAM !== 'undefined' && Array.isArray(PROGRAM))
    ? PROGRAM.find(d => d && d.id === rec.dayId) : null;
  const acts = day ? (activeDayActivities(day) || []) : [];
  const restIds = new Set(acts.filter(a => a && a.rest).map(a => a.id));
  if (!restIds.size) return true;
  return ids.some(id => !restIds.has(id));
}
// SCOPE OF THE ABOVE (#15). It does NOT extend to isSessionTouched(), which asks a
// different question — "did the athlete interact with this day" — and must keep
// answering it: an opened-but-empty day still autosaves an in-progress record, and
// that record is deliberately kept and shown UNFINISHED (Albin, 27 Aug).
// One knock-on, stated rather than hidden: weekTypeCounts() reads this function, so
// a cardio-only day now counts toward a consistency tier. That is the correct
// reading of "did the athlete train that day"; it is still a behaviour change.
...
// ── DELOAD TRIGGER (rewritten v0.9.18, Albin's call) ──────────────────────
// WAS: three consecutive non-zero weeks of RISING TONNAGE. Retired. Tonnage rises
// when you add weight, but equally when you add an exercise, log sets you had
// previously forgotten, or move to a machine you load heavier — so it answered a
// question nobody asked, and the banner had to describe it as "3 weeks of rising
// load" and hope that was true. Weekly Tonnage remains in Stats untouched: the
// chart, the numbers and the progression carousel are unaffected. Only the
// automatic prompt changed owner.
// IS: three consecutive weeks of full strength-day coverage since the last
// accepted deload — much closer to the 3:1 load:recover cycle in Training for
// the New Alpinism, and verifiable by looking at the week board.
const DELOAD_CONSISTENT_WEEKS = 3;

// The strength days are READ FROM THE PROGRAM, never hardcoded as D1–D4 (#5).
// Albin's split has four; a different program may have three or five, and the
// engine must not care. Same file runs both apps.
function deloadStrengthDayIds() {
  return (typeof PROGRAM !== 'undefined' && PROGRAM.length ? PROGRAM : [])
    .filter(d => d && d.type === 'training').map(d => d.id);
}

// A week qualifies when EVERY strength day has a FINISHED record in it.
// "Finished" is declared, not inferred (#42): autoSaveSessionToHistory() is the
// only writer of inProgress. v0.9.19: nothing in the engine writes autoTicked any
// more, but the check STAYS — a Google Drive restore or a JSON import can bring
// pre-v0.9.19 records back after the one-time purge has already marked itself done,
// so both kinds are excluded by reading a field rather than guessing from shape.
// No set-count threshold — pressing FINISH is a declared act, whereas "at least
// N sets" would be a number nobody derived (#28), and it would discount a real
// session where the weights simply were not logged.
// Note this is coverage, not a session count: one record exists per dayId per
// weekKey, so training D1 twice in a week cannot be represented and counts once.
function weekMeetsDeloadConsistency(wk) {
  const days = deloadStrengthDayIds();
  if (!days.length) return false;
  return days.every(id => state.history.some(h =>
    h && h.dayId === id && h.weekKey === wk && !h.inProgress && !h.autoTicked));
}

// Derived from the accepted list, never stored as its own field (#12).
function lastAcceptedDeloadWeek() {
  const acc = (state.deloadDecision && state.deloadDecision.accepted) || [];
  // weekKeys are zero-padded YYYY-Www, so lexicographic order IS chronological,
  // across the year boundary too ('2025-W52' < '2026-W01').
  return acc.length ? acc.slice().sort()[acc.length - 1] : null;
}

function checkDeload() {
  // The window is the three COMPLETED weeks before this one — not the current
  // week. A week cannot qualify until its last strength day is finished, so
  // including the current week would raise the banner mid-week and a Yes would
  // then deload the back half of the third hard week, cutting it short. Reading
  // completed weeks only means the banner appears from Monday and the deload
  // applies to the week it is offered for.
  const weeks = [];
  for (let i = DELOAD_CONSISTENT_WEEKS; i >= 1; i--) {
    const d = new Date(); d.setDate(d.getDate() - i * 7);
    weeks.push(getWeekKey(d));
  }
  // Accepting a deload is a floor: nothing at or before it is ever counted again.
  // This is what makes Yes a three-week pause — the window has to refill.
  const floor = lastAcceptedDeloadWeek();
  if (floor && weeks.some(w => w <= floor)) return false;
  return weeks.every(weekMeetsDeloadConsistency);
}
...
function openDeloadPrompt() {
  ...
  mEl.textContent = active
    ...
    : (checkDeload()
        ? DELOAD_CONSISTENT_WEEKS + ' full training weeks in a row — do you want to deload this week? Yes lowers the coach\u2019s suggested weights and stops it flagging a drop as a mistake.'
...
function setDeloadDecision(choice) {
  const wk = getWeekKey(new Date());
  // v0.9.18: carry the accepted list across the weekly overwrite, then add or
  // remove THIS week. Turning an accepted week off removes it, so the floor rolls
  // back to the deload before it and the counter resumes where it was — the
  // reason `accepted` is a list rather than one string.
  const acc = ((state.deloadDecision && state.deloadDecision.accepted) || [])
    .filter(w => w !== wk);
  if (choice === 'yes') acc.push(wk);
  acc.sort();
  state.deloadDecision = { weekKey: wk, choice, accepted: acc };
...
function updateDeloadNotices() {
  ...
  if (homeNotice) {
    ...
    homeNotice.innerHTML = showHome
      ? `<span>⚠️ Deload week due — ${DELOAD_CONSISTENT_WEEKS} full training weeks in a row</span>` : '';
...
// v0.9.21d — ONE PREDICATE, ANY WEEK. deloadDecision.accepted[] is the durable,
// declared list of every week actually deloaded (v0.9.18). Reading it here means
// a record can be stamped for the week it belongs to, and last week can be
// retro-flagged, without inferring anything from loads.
function isDeloadWeek(wk) {
  const d = state.deloadDecision;
  if (!d || !wk) return false;
  if (Array.isArray(d.accepted) && d.accepted.includes(wk)) return true;
  return !!(d.choice === 'yes' && d.weekKey === wk);
}
// "Active" is the BOARD's week, not the wall clock. They only differ while a
// rollover is pending — and that is exactly when it matters: finishing last
// week's deload sessions on Monday morning must still show deload targets.
function isDeloadActive() {
  return isDeloadWeek(state.currentWeekKey || getWeekKey(new Date()));
}
...
async function _doDriveRestore() {
  ...
  try {
    ...
    // v0.9.19: a restore can carry pre-release records back in. Runs BEFORE the
    // streak is painted, so the figure on screen matches the repaired history
    // rather than the number stored in the file (#12).
    repairRestoredHistory('drive restore');
    document.getElementById('streak-count').textContent = state.streak;
...
// v0.9.19 — this reported `0 B of 10240.93 MB` beside 66 real sessions. TWO faults,
// both introduced when v0.9.7a namespaced storage and this function was not revisited:
//   1. it matched keys starting with 'gymdolph_', but every key is now written as
//      'gd_<user>__gymdolph_...', so nothing ever matched and the loop did nothing;
//   2. even had it matched, it read values with the WRAPPED getItem(), which adds the
//      prefix again — 'gd_x__gd_x__gymdolph_history' — and returns null.
// The quota half was always right, which is why the row read 0 B of a real number and
// looked like a display bug rather than a scanner that found nothing (#13: validate on
// show — a total of zero beside 66 sessions should never have rendered as a fact).
// #19 again: the function had a consumer and was never read.
function storageBreakdown() {
  const rows = [];
  let total = 0;
  const NS  = window.__gdNS || '';
  const get = window.__gdRawGet || (k => localStorage.getItem(k));
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (!k || k.indexOf(NS + 'gymdolph_') !== 0) continue;   // this app's keys only
    // UTF-16 in most engines; 2 bytes per code unit is the honest upper bound.
    const bytes = ((get(k) || '')).length * 2 + k.length * 2;
    rows.push({ key: k.slice((NS + 'gymdolph_').length), bytes });
    total += bytes;
  }
...
function renderStorageIndicator() {
  ...
  // v0.9.19: was hist[hist.length - 1].date — the LAST ELEMENT, which is the earliest
  // record only while history stays perfectly sorted newest-first. Nothing enforces
  // that, and the number is presented to the user as a fact (#12). Derived now.
  const oldest = hist.reduce((min, h) =>
    (h && h.date && (!min || h.date < min)) ? h.date : min, null);
  ...
  if (navigator.storage && navigator.storage.estimate) {
    navigator.storage.estimate().then(est => {
      ...
      // v0.9.19: the bar has to move with the label. It was left filling against the
      // 5 MB assumption while the text beside it announced a 10 GB quota, so the two
      // halves of one reading disagreed about their denominator (#14). Recomputed
      // from the same number the user is shown.
      const p2  = Math.min(100, (total / est.quota) * 100);
      const lv2 = p2 > 80 ? 'crit' : (p2 > 60 ? 'warn' : 'ok');
      const fill = box.querySelector('.si-fill');
      if (fill) { fill.className = 'si-fill ' + lv2;
                  fill.style.width = Math.max(1.5, p2).toFixed(1) + '%'; }
      const tot = box.querySelector('.si-total');
      if (tot) tot.className = 'si-total ' + lv2;
      const note = box.querySelector('.si-note');
      if (note) note.className = 'si-note ' + lv2;
    }).catch(function(){});
  }
}
  ...
  reader.onload=function(ev){
    try{
      ...
      function doImport() {
        ...
        repairRestoredHistory('json import');   // v0.9.19 — see the drive restore
        document.getElementById('streak-count').textContent=state.streak;
        save(); renderHistory(); buildHomeDayCards();
...
function exerciseHistoryFor(name) {
  ...
  state.history.forEach(rec => {
    ...
    // v0.9.21d: a deload row is labelled so the model can tell a planned drop from
    // a real one when it reads the history back.
    if (found && found.weight) rows.push(`${rec.date}: ${found.weight}${rec.deload ? ' (deload week)' : ''}`);
  });
...
// v0.9.21c: the deload told to the model CONCRETELY, next to the numbers it has to
// explain. deloadCoachContext() states it in the system prompt and has since v0.9.12,
// and that was not enough: the user message carried 'PREV 50kg × 8' against today's
// 30kg with no mention of a deload anywhere, and the model wrote back "with no deload
// flagged this week" — confabulating against a system line it had been given. A
// generic claim above loses to concrete numbers below. So the flag travels WITH the
// numbers, in the exercise's own unit, and names this exercise's actual target.
function deloadExerciseLine(ex) {
  if (!isDeloadActive() || !ex) return '';
  const n = deloadWeightFor(ex.name);
  const isHold = logModeOf(ex) === 'hold';
  let target = '';
  if (n) {
    const val = isHold ? (n.seconds ?? n.weight) : (n.weight ?? n.seconds);
    if (val != null) target = ` Deload target for this exercise: ${val}${isHold ? 's' : 'kg'}` +
      (n.reps != null ? ` × ${n.reps}` : '') + '.';
  }
  return `\nDELOAD WEEK — flagged by the athlete for this week.${target}` +
         ` Loads, holds and volume below the PREV figures are deliberate, not a drop in performance.`;
}
function coachContextFor(exId) {
  const ex = findExById(exId);
  if (!ex) return '';
  const sess = getSession(state.workoutDay.id);
  return `Session elapsed: ${coachDurationLine()}${deloadExerciseLine(ex)}
Exercise: ${ex.name}
...
On a flagged deload week, a lower weight, a shorter hold or reduced volume is the plan being followed correctly — do not raise triggers 2, 3 or 4 for it. TRIGGER 1 STILL APPLIES IN FULL: a deload week never buys silence on a shoulder or lower-back constraint breach.
...
function updateHRFocusOverlay(bpm, zoneKey) {
  ...
  // v0.9.20: was `(bpm > 0) ? 'block' : 'none'`. A strap that has been taken off
  // sits at 0 bpm, so the one state in which you need to disconnect was the one
  // state in which the button was removed. An escape hatch must not be gated on
  // the condition it exists to escape. Shown whenever the overlay is open.
