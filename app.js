...
function finishWorkout() {
  ...
  if (editMode) {
    ...
    const today=todayStr(), yesterday=localDateStr(new Date(Date.now()-86400000));
...
const MILESTONE_LADDERS = [
  {key:'pullup',   title:'PROGRESSION',     icon:'🐬', cardLabel:'Pull-ups'},
    ...
    try{
      ...
      function doImport() {
        if(Array.isArray(data)){
          ...
          state.ytLinks=data.ytLinks||{};
          state.bodyStats=data.bodyStats||[];
          state.milestoneProgress=data.milestoneProgress||{};
          state.sessions=data.sessions||{};
          if(data.weekKey) state.currentWeekKey=data.weekKey;
        }
        document.getElementById('streak-count').textContent=state.streak;
