    ...
    if (block.type==='cardio') {
      ...
      block.exercises.forEach(ex => {
        ...
        const prevPerf = (block.type !== 'warmup' && hintText)
          ? `<span class="ex-prev"> / PREVIOUS PERF ${hintText}</span>` : '';
        ...
        exHTML += `<div class="exercise-row" id="exrow-${ex.id}"
          ...
          <div class="ex-info">
            ...
            ${(prescLine||prevPerf)?`<div class="ex-prescription">${prescLine}${prevPerf}</div>`:''}
...
function renderExportPanel() {
  ...
  if (ds.kind === 'body') {
    ...
    const tot = ds.rows.reduce((x,r)=>x+r.volume,0);
    const d = ds.rows[ds.rows.length-1].topWeight - ds.rows[0].topWeight;
    sEl.innerHTML = `<b>${ds.rows.length}</b> sessions · top set <b>${d>0?'+':''}${d}</b> kg · <b>${(tot/1000).toFixed(1)}</b> t volume`;
  }
}
...
function runExport(fmt) {
  ...
  if (ds.kind === 'body') {
    ...
    csv = 'date,exercise,category,sets,top_weight_kg,volume_kg,detail\n' + ds.rows.map(r =>
      [r.date,`"${r.exercise}"`,`"${r.category}"`,r.sets,r.topWeight,r.volume,`"${r.detail}"`].join(',')).join('\n');
  }
