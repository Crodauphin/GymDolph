    ...
    [state.notes.attempts, state.notes.deloadWeights].forEach(store => {
      Object.values(store).forEach(n => {
        if (n.isHoldSeconds === undefined) {
          ...
          n.isHoldSeconds = !!(progEx && progEx.holdSeconds);
  ...
  day.blocks.forEach(block => {
    ...
    if (block.type==='cardio') {
      ...
      block.exercises.forEach(ex => {
        ...
        const isHoldSeconds = !!ex.holdSeconds;
        ...
        if (block.type!=='warmup') {
          for (let s=0;s<ex.sets;s++) {
            ...
            const chipLabel = logged
              ? (isHoldSeconds ? `${logged.seconds}s` : (isBodyweight ? `${logged.reps} reps` : `${logged.weight}kg × ${logged.reps}`))
              : `Set ${s+1}`;
        ...
        // set/rep line — "4 × 4 / PREVIOUS PERF 22kg × 4". Warm-ups have no chips
        // and no meaningful history line, so they stay clean.
        const prevPerf = (block.type !== 'warmup' && hintText)
          ? `<span class="ex-sep"> / </span><span class="ex-prev">PREVIOUS PERF ${hintText}</span>` : '';
        ...
        let attemptHint = '';
        if (deload) {
          attemptHint = `<span class="ex-sep"> / </span><span class="ex-attempt ex-deload-attempt">${isHoldSeconds?'DELOAD':'DELOAD WEIGHT'} ${fmtHintValue(deload)}</span>`;
        } else if (attempt) {
          attemptHint = `<span class="ex-sep"> / </span><span class="ex-attempt">ATTEMPT ${fmtHintValue(attempt)} NEXT</span>`;
        }
...
function warmupComplete(sess) {
  if (!state.workoutDay) return false;
  const warm = (state.workoutDay.blocks || []).filter(b => b.type === 'warmup');
  const all = warm.flatMap(b => b.exercises || []);
  return all.length > 0 && all.every(e => sess.workoutChecks[e.id]);
}
...
function toggleExercise(exId, el) {
  ...
  // Session-timer auto-start. Two rules share one precondition — the timer has
  // never been started this session — and are mutually exclusive, evaluated in
  // this order so that ticking the LAST warm-up row wins over "first tick":
  //   v0.9.11 (#7) · warm-up now complete → start AT 10:00, because that's what the
  //                  warm-up block is scheduled to take and the 50/70-min milestones
  //                  should line up with the rest of the session.
  //   v0.9.5       · any other first warm-up tick → start at 0:00.
  // Deliberately literal: if the timer is already running (or has elapsed time),
  // neither fires — a clock the athlete started is never silently moved.
  if (s.workoutChecks[exId] && getTimerSecs() === 0 && !state.timerRunning) {
    ...
    if (block && block.type === 'warmup') {
      if (warmupComplete(s)) {
        state.timerPausedAt = 10 * 60;
        // The 10-min milestone would otherwise fire the instant the clock is set.
        state.notifiedMilestones[10 * 60] = true;
        startTimerTick();
        showTimerAutoToast('TIMER SET TO 10:00 — WARM-UP DONE');
      } else {
        startTimerTick();
        showTimerAutoToast();
      }
...
function openSetModal(dayId, blockId, exId, setIndex) {
  ...
  if (weightField) weightField.style.display = (isBodyweight || isHoldSeconds) ? 'none' : 'flex';
  ...
  if (repsInput) repsInput.placeholder = isHoldSeconds ? 'sec' : '0';
  ...
  if (prev) {
    ...
    document.getElementById('modal-reps').value   = isHoldSeconds ? (prev.seconds ?? '') : prev.reps;
...
function confirmSet() {
  ...
  const isHoldSeconds = ex ? !!ex.holdSeconds : false;
  const weight = (isBodyweight || isHoldSeconds) ? 0 : (parseFloat(document.getElementById('modal-weight').value)||0);
  ...
  sess.setLogs[exId][setIndex] = isHoldSeconds ? {seconds: repsOrSeconds} : {weight, reps: repsOrSeconds};
  sess.touched = true;
  ...
  if (chip) {
    ...
    chip.textContent = isHoldSeconds ? `${repsOrSeconds}s` : (isBodyweight ? `${repsOrSeconds} reps` : `${weight}kg × ${repsOrSeconds}`);
...
function setTimerTo(secs) {
  ...
  NOTIF_MILESTONES.forEach(ms => { state.notifiedMilestones[ms] = (ms <= secs); });
  updateHeaderTimer();
...
function openTimerSheet() {
  ...
  el.innerHTML = `<div class="modal-box ts-box">
      ...
      <div class="ts-grid">
        <button class="ts-jump" onclick="setTimerTo(600)">10 MIN<span>warm-up done</span></button>
        <button class="ts-jump" onclick="setTimerTo(3000)">50 MIN<span>keep going</span></button>
        <button class="ts-jump" onclick="setTimerTo(4200)">70 MIN<span>almost there</span></button>
      </div>
...
function autoSaveSessionToHistory(dayId) {
  ...
  if (day.type === 'rest') {
    ...
    day.blocks.forEach(b => b.exercises.forEach(ex => {
      ...
      const summary = ex.holdSeconds
        ? sets.filter(s=>s&&s.seconds!=null).map(s=>`${s.seconds}s`).join(', ')
...
function renderModifyBody(record) {
  ...
  (record.exercises||[]).forEach((ex,idx)=>{
    ...
    const isHoldSeconds = !!(ex.holdSeconds || (progEx && progEx.holdSeconds));
    let chips='';
    if (isCardio) {
      ...
      for(let s=0;s<numSets;s++){
        ...
        const label=logged?`${logged.seconds}s`:`Set ${s+1}`;
...
function openHistorySetModal(recordId, exId, setIndex) {
  ...
  const progEx = PROGRAM.flatMap(d=>(d.blocks||[]).flatMap(b=>b.exercises||[])).find(e=>e.id===exId);
  const isHoldSeconds = !!(ex.holdSeconds || (progEx && progEx.holdSeconds));
  state.historySetTarget={recordId, exId, setIndex, isHoldSeconds};
  const setLogs=record.setLogs||{};
  const prev=(setLogs[exId]||[])[setIndex];
  document.getElementById('modal-ex-name').textContent=`${ex.name} — Set ${setIndex+1}`;
  document.getElementById('modal-prefill-hint').textContent='';
  const weightField = document.getElementById('modal-weight-field');
  if (weightField) weightField.style.display = isHoldSeconds ? 'none' : 'flex';
  const repsLabelEl = document.getElementById('modal-reps-label');
  if (repsLabelEl) repsLabelEl.textContent = isHoldSeconds ? 'Seconds held' : 'Reps';
  document.getElementById('modal-weight').value=prev?prev.weight:'';
  document.getElementById('modal-reps').value = isHoldSeconds
    ? (prev ? (prev.seconds ?? '') : '')
    : (prev ? prev.reps : (ex.reps||''));
  document.getElementById('set-modal').style.display='flex';
  setTimeout(()=>document.getElementById(isHoldSeconds?'modal-reps':'modal-weight').focus(),100);
}
function confirmHistorySet() {
  const {recordId,exId,setIndex,isHoldSeconds}=state.historySetTarget||{}; if(!recordId) return;
  const record=state.history.find(r=>r.id===recordId); if(!record) return;
  ...
  record.setLogs[exId][setIndex] = isHoldSeconds ? {seconds: repsOrSeconds} : {weight, reps: repsOrSeconds};
  closeModal();
  state.historySetTarget=null;
  renderModifyBody(record);
  save();
...
function closeNotes() { showView('workout'); }
...
function renderNotes() {
  ...
  // Populate the exercise-name datalist from today's day, so both forms
  // can autocomplete instead of requiring an exact hand-typed match.
  const dl = document.getElementById('attempt-exname-list');
  if (dl && day) {
    const names = [];
    (day.blocks||[]).forEach(b => (b.exercises||[]).forEach(ex => names.push(ex.name)));
  ...
  if (deloadList) {
    deloadList.innerHTML = deloadKeys.length ? deloadKeys.map(key => {
      const dl2 = deloads[key];
      const k = key.replace(/'/g,"\\'");
      return `<div class="notes-item">
        <div class="notes-item-main">
          <div class="notes-item-title">${esc(dl2.exName)}</div>
          <div class="notes-item-sub">Deload ${dl2.isHoldSeconds?'hold':'weight'}: ${fmtNoteValue(dl2)} — clears itself after this week</div>
        </div>
    ...
    }).join('') : '<div class="notes-empty">No deload weights set for this week.</div>';
  }
  ...
  attemptList.innerHTML = attemptKeys.length ? attemptKeys.map(key => {
    const a = attempts[key];
    const k = key.replace(/'/g,"\\'");
    return `<div class="notes-item">
      <div class="notes-item-main">
        <div class="notes-item-title">${esc(a.exName)}</div>
        <div class="notes-item-sub">Target: ${fmtNoteValue(a)}${a.date ? ' · ' + esc(a.date) : ''}</div>
      </div>
      <div class="notes-item-actions">
  ...
  }).join('') : '<div class="notes-empty">No attempt targets set.</div>';
  ...
  generalList.innerHTML = general.length ? general.map(n => `
    <div class="notes-item">
      <div class="notes-item-main">
        <div class="notes-item-sub">${esc(n.date)}</div>
...
function addAttemptNote() {
  ...
  if (!exName || !value) { appAlert('MISSING INFO', 'Enter both an exercise name and a target.'); return; }
  ...
  const progEx = findProgramExercise(exName);
  const isHoldSeconds = !!(progEx && progEx.holdSeconds);
  const key = normExName(exName);
  if (!state.notes.attempts) state.notes.attempts = {};
...
function addDeloadWeightNote() {
  ...
  const value = parseFloat(valueEl.value);
  const reps = repsEl.value !== '' ? parseInt(repsEl.value) : null;
  if (!exName || !value) { appAlert('MISSING INFO', 'Enter both an exercise name and a deload target.'); return; }
  const progEx = findProgramExercise(exName);
  const isHoldSeconds = !!(progEx && progEx.holdSeconds);
  const key = normExName(exName);
  if (!state.notes.deloadWeights) state.notes.deloadWeights = {};
  ...
  body.innerHTML = ladders.map(l=>{
    ...
    const cards = steps.map((st,i)=>{
      ...
      return `<div class="ms-card${done?' done':''}${isCur?' current':''}${nudge?' nudge':''}${rep?' repeating':''}"
        ...
        ${(done && !rep) ? `<div class="ms-stamp"><span class="ms-stamp-tick">✓</span><span class="ms-stamp-date">${prog[id].date}</span></div>` : ''}
      </div>`;
    }).join('');
...
function openDeloadPrompt() {
  ...
  const mEl = document.getElementById('app-dialog-msg');
  const bEl = document.getElementById('app-dialog-btns');
  tEl.textContent = 'Deload this week?';
  mEl.textContent = 'Rising load over the last 3 weeks — do you want to deload this week? Yes lowers the coach\u2019s suggested weights and stops it flagging a drop as a mistake.';
  ...
  document.getElementById('deload-no-btn').onclick  = () => { setDeloadDecision('no');  closeAppDialog(); };
  document.getElementById('deload-yes-btn').onclick = () => { setDeloadDecision('yes'); closeAppDialog(); };
  el.style.display = 'flex';
}
...
function fmtSets(logs, isHoldSeconds) {
  if (isHoldSeconds) {
    return (logs||[]).filter(s=>s&&s.seconds!=null)
      .map(s=>`${s.seconds}s held`).join(', ') || 'none yet';
  }
...
function todaysOtherSets(excludeExId) {
  ...
  state.workoutDay.blocks.forEach(b => (b.exercises||[]).forEach(e => {
    ...
    const f = fmtSets((sess.setLogs||{})[e.id], !!e.holdSeconds);
...
Today's sets for this exercise: ${fmtSets((sess.setLogs||{})[ex.id], !!ex.holdSeconds)}
    ...
    (r.exercises||[]).forEach(ex => {
      ...
      if (hsSets.length) {
        rows.push({
          ...
          detail: hsSets.map(s=>`${s.seconds}s`).join(' '),
        });
        return;
      }
      const sets = setLogsFor(r, ex.name);
      ...
      rows.push({
        ...
        volume: sets.reduce((a,s)=>a + s.weight*(s.reps||0), 0),
...
function renderExportPanel() {
  ...
  if (ds.kind === 'body') {
    ...
    const weightRows = ds.rows.filter(r=>r.mode!=='seconds');
...
function runExport(fmt) {
  ...
  if (ds.kind === 'body') {
    ...
    csv = 'date,exercise,category,sets,top_weight_kg,volume_kg,top_seconds,detail\n' + ds.rows.map(r =>
      [r.date, `"${r.exercise}"`, `"${r.category}"`, r.sets,
       r.mode==='seconds' ? '' : r.topWeight,
       r.mode==='seconds' ? '' : r.volume,
