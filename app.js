...
function showTimerAutoToast() {
  ...
  if (!toast) {
    ...
    toast.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);' +
      ...
      'padding:8px 18px;font-family:\'Rajdhani\',sans-serif;font-size:12px;font-weight:600;' +
...
function renderBodyStatsSummary() {
  ...
  if (!state.bodyStats.length) {
    if (el) el.innerHTML = '';
    if (emptyEl) emptyEl.style.display = 'block';
    return;
  }
  if (emptyEl) emptyEl.style.display = 'none';
  // Find most recent entry that has each metric independently
  ...
  el.innerHTML = `
    ${latestWeight ? `<div class="body-stat-card" onclick="openBodyStatsModal('${latestWeight.date}')" style="cursor:pointer">
      <div class="body-stat-label">BODY WEIGHT</div>
      <div><span class="body-stat-value">${latestWeight.weight}</span><span class="body-stat-unit">kg</span></div>
      <div class="body-stat-date">${latestWeight.date}</div>
    </div>` : ''}
    ${latestBF ? `<div class="body-stat-card" onclick="openBodyStatsModal('${latestBF.date}')" style="cursor:pointer">
      <div class="body-stat-label">BODY FAT</div>
      <div><span class="body-stat-value">${latestBF.bf}</span><span class="body-stat-unit">%</span></div>
      <div class="body-stat-date">${latestBF.date}</div>
    </div>` : ''}
    ${latestMuscle ? `<div class="body-stat-card" onclick="openBodyStatsModal('${latestMuscle.date}')" style="cursor:pointer">
      <div class="body-stat-label">MUSCLE MASS</div>
      <div><span class="body-stat-value">${latestMuscle.muscle}</span><span class="body-stat-unit">kg</span></div>
      <div class="body-stat-date">${latestMuscle.date}</div>
    </div>` : ''}`;
}
...
function renderBodyStatsTendency() {
  ...
  if (!el) return;
  if (state.bodyStats.length < 2) { el.style.display = 'none'; return; }
  const now = new Date();
  const cutoff = new Date(now); cutoff.setMonth(cutoff.getMonth() - 3);
  const cutoffStr = cutoff.toISOString().slice(0, 10);
  const recent = state.bodyStats.filter(e => e.date >= cutoffStr);
  if (recent.length < 2) { el.style.display = 'none'; return; }
  const newest = recent[0];
  const oldest = recent[recent.length - 1];
  function pct(now, then) {
    if (now == null || then == null || then === 0) return null;
    return ((now - then) / Math.abs(then) * 100).toFixed(1);
  }
  function arrow(v) { return v > 0 ? '↑' : '↓'; }
  function chip(label, val, invert) {
    if (val == null) return '';
    const positive = invert ? val < 0 : val > 0; // invert=true: decrease is good (weight/bf)
    const color = positive ? '#77FD01' : '#EB47CE';
    return `<span style="display:inline-flex;align-items:center;gap:4px;margin-right:10px;font-family:'Exo 2',sans-serif;font-size:13px;">
      ...
      <span style="color:${color};font-weight:600;">${arrow(val)}${Math.abs(val)}%</span>
    </span>`;
  }
  const wPct  = pct(newest.weight, oldest.weight);
  const bfPct = pct(newest.bf,     oldest.bf);
  const mPct  = pct(newest.muscle, oldest.muscle);
  const chips = [chip('WEIGHT', wPct, true), chip('B-F', bfPct, true), chip('M-MASS', mPct, false)].join('');
  if (!chips.trim()) { el.style.display = 'none'; return; }
  el.style.display = 'block';
  el.innerHTML = `<div style="font-family:'Rajdhani',sans-serif;font-size:10px;letter-spacing:1px;color:var(--text-dim);margin-bottom:6px;">LAST 3 MONTHS</div><div>${chips}</div>`;
}
...
function renderBodyStatsHistory() {
  ...
  const rows = open ? entries.map(e=>{
    ...
    <div class="bs-history-row" onclick="openBodyStatsModal('${e.date}')">
      ...
      <span class="bs-history-vals">${parts || '—'}</span>
    </div>`;
  }).join('') : '';
  el.innerHTML=`
...
function updateHRZoneSettings() {
  ...
  if (table) {
    const rows = [
      ...
      ['Recovery',   z.recovery  + '–' + (z.aerobic-1),   '#7EC8E3'],
      ['Aerobic',    z.aerobic   + '–' + (z.anaerobic-1), '#BEFF89'],
      ['Anaerobic',  z.anaerobic + '–' + (z.vo2max-1),    '#DC96D0'],
      ['VO2 Max',    z.vo2max + '+',                  '#FF6B6B'],
    ];
    table.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 16px;margin-top:4px;">' +
      rows.map(([name, range, colour]) =>
        `<span style="color:${colour};font-family:'Rajdhani',sans-serif;font-weight:700;letter-spacing:1px;">${name}</span>` +
        `<span style="color:var(--text-dim);font-family:'Rajdhani',sans-serif;">${range} bpm</span>`
      ).join('') +
      `<span style="color:var(--text-dim);grid-column:1/-1;margin-top:4px;">Max HR: ${z.max} bpm · based on 220 − age</span>` +
      '</div>';
  }
}
