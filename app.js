...
function autoCloseFinishedWeeks() {
  try {
    ...
    PROGRAM.forEach(day => {
      ...
      // v0.9.18: the FIFTH record-construction site, found by the regression smoke
      // when it noticed two different key counts in one history. Same treatment as
      // autoTickRestDay(): built by buildHistoryRecord() so the key set matches
      // every other record, with duration passed in as zero rather than derived —
      // sessionDurationParts() reads the LIVE clock, and this runs at boot, when a
      // timer left running for another day would otherwise be billed to a swept
      // passive day of last week.
      const rec = buildHistoryRecord(day, emptySessionShape(), {
        prev: null, id: Date.now() + added, date: dt, weekKey: closedWk,
        durationParts: { strength: 0, cardio: 0, total: 0, clamped: false },
      });
      rec.autoTicked = true;      // declared kind — the deload counter reads this (#42)
      rec.autoLabel  = label;
...
function autoTickRestDay() {
  ...
  // v0.9.18: the FOURTH record-construction site, found during the B5 work. It
  // was hand-built and carried neither the duration split nor any of the cardio /
  // active / yoga fields, so a rest-day record had its own key set.
  // durationParts is passed EXPLICITLY as zero: sessionDurationParts() reads the
  // live clock, so deriving it here would bill a running timer — one started for
  // another day and never stopped — to an auto-ticked rest day.
  const record = buildHistoryRecord(day, emptySessionShape(), {
    prev: prev, id: Date.now(), date: recDate, weekKey: state.currentWeekKey,
    durationParts: { strength: 0, cardio: 0, total: 0, clamped: false },
  });
  record.autoTicked = true;   // declared kind, read by the deload counter (#42)
...
// only writer of inProgress, and autoTickRestDay() the only writer of autoTicked,
