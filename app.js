...
function renderModifyBody(record) {
  ...
  dateRow.innerHTML=`<label style="font-size:11px;letter-spacing:1px;color:var(--text-dim);font-family:'Rajdhani',sans-serif;font-weight:700;">DATE</label>
    <input type="date" id="modify-date-input" value="${record.date||''}"
      ...
      padding:6px 10px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:15px;font-weight:600;outline:none;flex:1"/>`;
  body.appendChild(dateRow);
  (record.exercises||[]).forEach((ex,idx)=>{
    const setLogs=record.setLogs||{};
    const exSets=setLogs[ex.id]||[];
    // Set count: use logged count, exercise definition, or minimum 1
    // Show max of: sets defined on exercise, sets already logged, or 1
    const numSets = Math.max(ex.sets || 1, exSets.length || 0, 1);
...
function renderBodyStatsMetricHistory() {
  ...
  function recentFor(metric, unit, label) {
    ...
    return `<div style="flex:1;min-width:0">
      <div style="font-family:'Rajdhani',sans-serif;font-weight:700;font-size:10px;letter-spacing:1px;color:var(--text-dim);margin-bottom:4px">${label}</div>
      ${rows}
    </div>`;
  }
  ...
  if (table) {
    ...
    table.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px;margin-top:4px;">' +
      rows.map(([name, range, colour]) =>
        `<span style="color:${colour};font-family:'Rajdhani',sans-serif;font-weight:700;font-size:13px;letter-spacing:1px;">${name}</span>` +
        `<span style="color:var(--text-dim);font-family:'Rajdhani',sans-serif;font-weight:600;font-size:13px;">${range} bpm</span>`
      ).join('') +
