...
function getSession(dayId) {
  if (!state.sessions[dayId]) {
    state.sessions[dayId] = { setLogs: {}, workoutChecks: {}, cardioChecks: {}, cardioLogs: {}, cardioChoice: {}, touched: false };
  }
...
function restHintFor(blockType) {
      ...
      block.exercises.forEach(ex => {
        ...
        exHTML += `<div class="exercise-row" id="exrow-${ex.id}"
          ...
          <div class="ex-info">
            ...
            ${(block.type!=='warmup' && ex.sets>1)?`<div class="set-rest-hint">⏱ Rest ${restHintFor(block.type)} between sets</div>`:''}
          </div>
        </div>`;
      });
    }
