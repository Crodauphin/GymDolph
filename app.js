...
function renderBodyStatsTendency() {
  ...
  el.innerHTML = `<div style="font-family:'Rajdhani',sans-serif;font-size:10px;letter-spacing:1px;color:var(--text-dim);margin-bottom:6px;">LAST 3 MONTHS</div><div>${wChip}${bfChip}${mChip}</div>`;
}
...
function drawDualLineChart(canvas, labels, weights, bfs, tall, muscles) {
  ...
  if(validW.length){
    ...
    weights.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),yw(v),3,0,Math.PI*2);ctx.fillStyle='#77FD01';ctx.fill();});
    ...
    if(muscles && muscles.some(v=>v!=null)){
      ...
      ctx.strokeStyle='#BEFF89'; ctx.lineWidth=0.75; ctx.lineJoin='round'; ctx.setLineDash([4,3]); ctx.stroke();
      ctx.setLineDash([]);
      muscles.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),yw(v),2,0,Math.PI*2);ctx.fillStyle='#BEFF89';ctx.fill();});
    }
  }
  ...
  if(bfs && bfs.some(v=>v!=null)){
    ...
    ctx.strokeStyle='#EB47CE'; ctx.lineWidth=0.75; ctx.lineJoin='round'; ctx.stroke();
    bfs.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),ybf(v),2,0,Math.PI*2);ctx.fillStyle='#EB47CE';ctx.fill();});
  ...
  let lx=p.left;
  ctx.fillStyle='#77FD01'; ctx.fillText('● Weight',lx,14); lx+=60;
  if(bfs&&bfs.some(v=>v!=null)){ctx.fillStyle='#EB47CE'; ctx.fillText('● Body fat',lx,14); lx+=65;}
  if(muscles&&muscles.some(v=>v!=null)){ctx.fillStyle='#BEFF89'; ctx.fillText('● Muscle mass',lx,14);}
}
...
function updateHRZoneSettings() {
  ...
  if (table) {
    const rows = [
      ...
      ['Anaerobic',  z.anaerobic + '–' + (z.vo2max-1),    '#BEFF89'],
