...
function warmupJumpSecs(day) { const w = phaseMarks(day).find(m => m.type === 'warmup'); return w ? w.secs : 0; }
...
function maybeAutoStartSessionTimer() {
  ...
  const jump = warmupComplete(sess) ? warmupJumpSecs(day) : 0;
  if (jump > 0) {
    state.timerPausedAt = jump;
    sess.timerJumpSecs  = jump;           // credited back into the span ceiling
    // Marks at or below the jump are treated as seen, or they all fire at once.
    phaseMarkSecs(day).forEach(ms => { if (ms <= jump) state.notifiedMilestones[ms] = true; });
    startTimerTick();
    showTimerAutoToast('TIMER SET TO ' + pad2(Math.floor(jump/60)) + ':' + pad2(jump%60) + ' \u2014 WARM-UP DONE');
  } else {
    startTimerTick();
    showTimerAutoToast();
  }
...
function autoSaveSessionToHistory(dayId) {
  ...
  const _keep    = (!_dp.total && _prev && _prev.duration > 0);
  const duration = _keep ? _prev.duration : _dp.total;
  ...
  const record = {
    ...
    durationStrength: _keep ? (_prev.durationStrength || 0) : _dp.strength,
    durationCardio:   _keep ? (_prev.durationCardio   || 0) : _dp.cardio,
...
function finishWorkout() {
  ...
  if (editMode) {
    ...
    if (idx >= 0) {
      ...
      const _pr    = state.history[idx];
      const _keepE = (!_dp.total && _pr.duration > 0);
      const _durE  = _keepE ? _pr.duration : duration;
      state.history[idx] = {
        ...state.history[idx],
        duration: _durE, durationStr: formatDuration(_durE),
        durationStrength: _keepE ? (_pr.durationStrength || 0) : _dp.strength,
        durationCardio:   _keepE ? (_pr.durationCardio   || 0) : _dp.cardio,
        durationEstimated: _keepE ? !!_pr.durationEstimated : false,
        setLogs: JSON.parse(JSON.stringify(sess.setLogs || {})),
        checks:  JSON.parse(JSON.stringify(sess.workoutChecks || {})),
    ...
    const record = {
      ...
      dateStr:  recordDateStr(recDate),
      duration, durationStr:formatDuration(duration),
      durationStrength: _dp.strength, durationCardio: _dp.cardio, durationEstimated: false,
      setLogs:  JSON.parse(JSON.stringify(sess.setLogs || {})),
      checks:   JSON.parse(JSON.stringify(sess.workoutChecks || {})),
      activeLog: JSON.parse(JSON.stringify(sess.activeLog || {})),
      yogaSessionLog: JSON.parse(JSON.stringify(sess.yogaSessionLog || {duration:null,style:null})),
