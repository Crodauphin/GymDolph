...
function saveBodyStats() {
  ...
  save();
  driveBackup(true); // v0.9: auto-backup after logging body stats
  closeBodyStatsModal();
...
// Replace GYMDOLPH_CLIENT_ID with your OAuth2 client ID from Google Cloud Console.
...
// Find or create the backup file in Drive root
async function driveGetOrCreateFileId(token) {
  if (driveState.fileId) return driveState.fileId;
  // Search Drive root for existing backup file
  const search = await fetch(
    `https://www.googleapis.com/drive/v3/files?q=name='${BACKUP_FILENAME}' and trashed=false&fields=files(id)`,
    { headers: { Authorization: 'Bearer ' + token } }
  );
  ...
  const meta = { name: BACKUP_FILENAME };
