...
function renderSubHeader(){
  ...
  var w=document.getElementById('sh-weight'); if(w) w.textContent=last?(last.weight+' kg'):'— kg';
  var bf=document.getElementById('sh-bf'); if(bf) bf.textContent=(last&&last.bf)?(last.bf+'% bf'):'—% bf';
}
...
function showMilestone(time, msg, color) {
  ...
  try {
    ...
    osc.frequency.value = color === 'green' ? 523 : 440; // C5 for 10min, A4 for 50/70
    osc.type = 'sine';
...
function startRestTimer(s) {
  ...
  state.restInterval = setInterval(() => {
    ...
    document.getElementById('widget-ring-fg').style.strokeDashoffset = 232.48*(1-state.restRemaining/state.restTotal);
...
// Zones based on % of max HR (220 - age), Karvonen-style boundaries:
// Recovery:   50–60% max HR
// Aerobic:    60–70% max HR
// Anaerobic:  70–80% max HR
// VO2 Max:    80%+ max HR
...
function calcHRZones(age) {
  ...
  return {
    ...
    anaerobic:  Math.round(max * 0.70),
    vo2max:     Math.round(max * 0.80),
  };
}
...
function updateHRZoneSettings() {
  ...
  _hrZoneThresholds = { recovery: z.aerobic, aerobic: z.anaerobic, anaerobic: z.vo2max };
  // Render zone table
  if (table) {
    const rows = [
...
function ecgAmpFrame(ts) {
  ...
  _ecgLastTs = ts;
  // Lerp amplitude — reaches target in ~500ms
  _ecgAmp += (_ecgTargetAmp - _ecgAmp) * Math.min(6 * dt, 1);
  if (Math.abs(_ecgTargetAmp - _ecgAmp) > 0.1) {
    ...
    trace.setAttribute('d', buildSinePath(_ecgAmp));
    _ecgRafId = null; // done lerping, CSS scroll continues independently
  }
}
