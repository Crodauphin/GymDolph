...
function getSession(dayId) {
  if (!state.sessions[dayId]) {
    state.sessions[dayId] = { setLogs: {}, workoutChecks: {}, cardioChecks: {}, cardioLogs: {}, cardioChoice: {}, cardioSelected: {}, commuteChecked: false, commuteLog: null, _cardioDefaultsApplied: false, touched: false };
  }
    ...
    if (!isSel) {
      return `<div class="exercise-row" onclick="toggleCardioPoolItem('${item.id}')">
        ...
        <div class="ex-info">
          ...
          <div class="ex-prescription">${item.prescription}</div>
        </div>
      </div>`;
    }
...
function confirmSet() {
  ...
  // v0.9.8: rest duration priority —
  //   1. per-exercise `restSeconds` override (curated, e.g. DB shoulder press → 90s)
  //   2. your last manually-picked duration (persisted across app restarts)
  //   3. the generic block-type default
  ...
  if (hasOverride) {
    suggestedDur = ex.restSeconds; suggestedLbl = ex.name || '';
  } else if (hasRemembered) {
    suggestedDur = state.lastManualRestDuration; suggestedLbl = 'your last pick';
  } else {
    suggestedDur = restDefaults[blockId] !== undefined ? restDefaults[blockId] : 60;
    suggestedLbl = restLabels[blockId] || '';
  }
...
function resumeSessionFromHistory(recordId) {
  ...
  // v0.9.8: best-effort reconstruction of cardio selections (incl. commute, now
  // just another pool id) from the saved `exercises` list — history doesn't store
  // structured cardio logs, so per-item numeric fields aren't recoverable here,
  // only the tick state is.
  const cardioSelected = {};
  (record.exercises||[]).forEach(ex=>{
    if (!ex.checked) return;
    if (window.PROGRAM.cardioPool && window.PROGRAM.cardioPool.some(p=>p.id===ex.id)) {
      cardioSelected[ex.id] = true;
    }
  });
  state.sessions[day.id] = {
    ...
    cardioSelected,
    cardioLogs:    {},
    _cardioDefaultsApplied: true, // selections already resolved above — don't re-apply the commute default
    touched:       true,
  };
  state.workoutDay        = day;
